...
function testUiAndBillingContracts() {
  ...
  assert.match(storyboard, /aspect-ratio:\$\{Number\(ratio\[1\]\)\} \/ \$\{Number\(ratio\[2\]\)\}/);
  assert.match(css, /\.sketch-actions \{ display: flex; justify-content:center/);
...
console.log(JSON.stringify({ passed: true, checks: 36, paid_model_calls: 0, scene_camera_coordinates: '4/4', person_blocking_coordinates: '1/1', immediate_storyboard_progress: true, billing_unknown_requires_explicit_user_click: true, native_browser_dialogs: 0, director_button_min_width: 152 }));
