...
assert.equal(store.resumeStep({ stage: 'blueprint_failed' }, {}), 3, '剧本失败且没有保存结果时必须回到第 3 步');
assert.equal(store.resumeStep({ stage: 'blueprint_failed' }, { blueprint: { beats: [{ beat_index: 1 }] } }), 4, '重新生成失败但旧剧本仍存在时可以继续查看旧剧本');
...
assert.equal(store.resumeStep({ stage: 'media_failed' }, { video_clips: [{ video_url: '/shot.mp4' }] }), 6);
assert.equal(store.resumeStep({ stage: 'tts_ready' }, { tts_audio: { tracks: [] } }), 6);
assert.equal(store.resumeStep({ stage: 'compose_failed' }, {}), 6);
...
assert.equal(store.resumeStep({}, { final_video: { video_url: '/video.mp4' } }), 6);
