#!/usr/bin/env node
/**
 * 2026-04-25 部署：登录路由统一 + 后台统计补人民币按品类拆分
 *                + 强制 KB 默认开 + 飞书提示词导入 UI
 *                + 阿里云智能语音 2.0 永久 token 文案
 *
 * 用法（Git Bash）：
 *   VIDO_DEPLOY_HOST=43.98.167.151 VIDO_DEPLOY_PASSWORD='xxxxx' \
 *     node scripts/deploy-2026-04-25.js
 */
...
(async () => {
  ...
  console.log(`\n✓ 上传 ${uploaded}/${FILES.length} 个文件 (缺失 ${missing})`);
  ...
  // 看一下 KB seed 日志
  const r4 = await runExec(c, `pm2 logs ${PM2_APP} --lines 40 --nostream 2>&1 | grep -E "KB|seed|premium|listening|started|Error" | tail -20`);
  console.log('\n▶ 启动日志要点:');
  console.log((r4.out || '(无匹配)').trim());
