...
assert.match(personDossierStyles, /\.person-detail-tabs/u, '人物详情按需样式必须包含标签页布局');
assert.match(personDossierStyles, /\.person-image-category/u, '人物详情按需样式必须包含分类图片布局');
...
assert.doesNotMatch(storyboard, /shot-inline-editor|data-save-inline-shot|data-edit-shot/, '内部文字镜头合同不得作为分镜主界面的编辑步骤');
assert.match(storyboard, /friendlyBindings/, '绑定资产必须显示用户可理解的名称而非裸 ID');
assert.doesNotMatch(storyboard, /镜头详情|shotPromptPreview|storyboard-complete-row/, '分镜主界面不得暴露内部制作字段表');
assert.match(storyboard, /系统会根据已确认的剧情自动匹配人物与场景，直接生成分镜画面/, '分镜主界面必须说明系统自动匹配资产');
...
assert.match(storyboard, /保存参考视频提取的分镜草稿后，系统会继续自动匹配人物与场景/, '参考逐镜只需保存后交由系统继续绑定');
assert.doesNotMatch(storyboard, /data-open-shot-design/, '正式分镜主流程不得要求用户进入镜头设计二次确认');
...
const sketchOrder = ['data-generate-sketch', 'data-upload-sketch'].map(token => sketchActions.indexOf(token));
assert(sketchOrder.every(index => index >= 0), '分镜调整区必须集中保留本镜重生和上传替换');
assert.deepEqual([...sketchOrder].sort((a, b) => a - b), sketchOrder, '分镜调整区的 DOM/键盘顺序必须为生成、上传');
assert.doesNotMatch(sketchActions, /data-confirm-sketch|data-skip-sketch/, '分镜图片生成后不得再要求确认或跳过');
