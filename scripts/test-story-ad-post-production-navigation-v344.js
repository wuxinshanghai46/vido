...
assert.match(composeView, /const framesReady = shots\.length > 0 && approvedFrames\.length >= shots\.length/, '必须全部首帧就绪后才能显示视频生成入口');
assert.match(composeView, /data-back-storyboard/, '首帧不完整时必须显示返回人物场景分镜入口');
assert.match(composeView, /data-empty-action="back-storyboard"/, '视频空状态必须提供可操作的分镜返回按钮');
