...
assert.match(serviceSource, /if \(!assetPlan\.complete\(current, ctx\)\)[\s\S]*assetPlan\.generate/, 'Active Plan 缺失时必须走完整方案自举');
...
console.log(JSON.stringify({ passed: true, checks: 15, paid_model_calls: 0, media_calls: 0 }));
