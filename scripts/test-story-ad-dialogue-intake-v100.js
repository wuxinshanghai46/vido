...
async function main() {
  ...
  assert.deepEqual(
    dialogue.dialogueIntakeState({ name: '完整项目', mode: 'narrative_story', idea: '完整剧情', referenceAttached: true }),
    { ready: true, missing: [], next: '' },
    '已经附加参考材料的完整输入不得重复追问参考材料',
  );
  assert.deepEqual(
    dialogue.dialogueIntakeState({ name: '无参考项目', mode: 'commercial_subject', idea: '完整广告要求', referenceSkipped: true }),
    { ready: true, missing: [], next: '' },
    '用户明确选择无参考材料后必须可以继续',
  );
  assert.deepEqual(
    dialogue.dialogueIntakeState({ name: '待补参考项目', mode: 'commercial_subject', idea: '完整广告要求' }),
