...
const FILES = [
  'src/server.js',           // /login.html 路由从 redirect 改为 sendFile
  'public/login.html',       // 恢复 admin 独立登录页 + ?redirect= 支持
  'public/js/auth.js',       // requireAdmin 跳 /login.html 不跳首页
  'public/js/home.js',       // 已登录访问 / 自动跳工作台
  'public/replicate.html',   // "返回工作台" 改 /index.html
];
...
(async () => {
  ...
  console.log((await exec(c, 'curl -s -o /dev/null -w "login=%{http_code}\\n" http://127.0.0.1:4600/login.html')).trim());
  c.end();
