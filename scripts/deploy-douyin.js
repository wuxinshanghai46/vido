...
const FILES=['src/services/douyinExtract.js','src/services/radarService.js'];
...
(async()=>{
  ...
  // 测试新路径
  console.log('==test==');
  console.log((await ex(c,`TOKEN=$(curl -s -X POST http://127.0.0.1:4600/api/auth/login -H "Content-Type: application/json" -d '{"username":"admin","password":"admin123"}' | node -e "let d='';process.stdin.on('data',c=>d+=c).on('end',()=>{try{console.log(JSON.parse(d).data.access_token||'')}catch{console.log('')}})") && curl -s -w "\n[%{http_code}]\n" -X POST "http://127.0.0.1:4600/api/radar/extract?token=$TOKEN" -H "Content-Type: application/json" -d '{"url":"https://v.douyin.com/JmrzKWnn-go/"}' | head -c 600`)).trim());
  c.end();
