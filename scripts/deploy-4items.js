...
function _fileChecked() { return FILES; }
...
(async()=>{
  ...
  for(const r of FILES){await up(sf,path.join(REPO,r),path.posix.join('/opt/vido/app',r.split(path.sep).join('/')));console.log('  ↑',r);}
  ...
  // 测试预设音色试听
  console.log('==test preview-voice longxiaochun==');
  console.log((await ex(c,`TOKEN=$(curl -s -X POST http://127.0.0.1:4600/api/auth/login -H "Content-Type: application/json" -d '{"username":"admin","password":"admin123"}' | node -e "let d='';process.stdin.on('data',c=>d+=c).on('end',()=>{try{console.log(JSON.parse(d).data.access_token||'')}catch{console.log('')}})") && curl -s -o /tmp/preview.mp3 -w "http=%{http_code} size=%{size_download} ct=%{content_type}\\n" -X POST "http://127.0.0.1:4600/api/avatar/preview-voice?token=$TOKEN" -H "Content-Type: application/json" -d '{"text":"测试","voiceId":"longxiaochun"}' && file /tmp/preview.mp3 | head -1`)).trim());
  c.end();
