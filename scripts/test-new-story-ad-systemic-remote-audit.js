...
assert(command.includes('/opt/vido/current') && command.includes('run-with-pm2-env.js vido node -e'), 'audit command must execute read-only code in the active immutable release with production PM2 environment');
assert(!command.includes("cd '/opt/vido/app'"), 'audit must not inspect the retired mutable app tree after immutable cutover');
