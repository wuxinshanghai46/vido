...
async function main() {
  ...
  const mixedPath = mediaAdapter.assetPathFromName(decodeURIComponent(mixed.audio_url.split('/').pop()));
  assert(fs.existsSync(mixedPath) && fs.statSync(mixedPath).size > 1000, '必须生成可播放的本地混合音频');
  const cached = await mixPreview.create(taskId, { voice_volume: 0.8, bgm_volume: 0.12 });
  assert.strictEqual(cached.cached, true, '相同素材和音量必须复用本地混合试听，不能重复处理');
  ...
  console.log(JSON.stringify({
    ...
    mixed_preview_bytes: fs.statSync(mixedPath).size,
