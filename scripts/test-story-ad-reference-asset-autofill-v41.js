...
const historyMode = new Function(`${historyModeSource}; return { historicalStepReadOnly, historicalStepUsesGlobalEdit, applyHistoricalReadonlyControls };`)();
...
assert.match(appView, /historicalStepUsesGlobalEdit\(route\)[\s\S]*data-unlock-history-step/, '只有目标与材料历史步骤使用全局显式编辑入口');
...
assert.equal(historyMode.historicalStepUsesGlobalEdit({ view: 'brief' }), true, '目标与材料保留全局新增/修改入口');
assert.equal(historyMode.historicalStepUsesGlobalEdit({ view: 'assets' }), false, '人物资产必须使用本步骤的独立编辑入口');
