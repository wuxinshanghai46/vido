...
(async()=>{
 ...
 const feedback=await load(read('public/story-ad/components/videoGenerationFeedback.js'));
 ...
 assert.match(ui.generationProgressPanel(base,'compose'),/role="alert"/);
 ...
 const app=read('public/story-ad/app.js');assert.match(app,/route.view === 'compose' \? videoGenerationFeedbackMarkup/);assert.match(app,/currentRoute\(\).view === 'compose' \? videoGenerationFeedbackMarkup/);assert.match(app,/syncVideoGenerationControls\(store.state.bundle/);
 ...
 try{
  ...
  await page.addScriptTag({content:read('public/story-ad/components/videoGenerationFeedback.js').replace(/export /g,'')});
