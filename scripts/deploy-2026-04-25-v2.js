#!/usr/bin/env node
/**
 * 2026-04-25 v2 部署：去阿里 token 卡片 + 视频首帧 poster + 视频克隆需求文档
 */
...
const FILES = [
  ...
  // === 视频克隆产品需求文档 ===
  'docs/reports/2026-04-25-video-clone-product-analysis.md',
];

const REPO_ROOT = path.resolve(__dirname, '..');

function connect() { return new Promise((resolve, reject) => { const c = new Client(); c.on('ready', () => resolve(c)); c.on('error', reject); c.connect({ host: HOST, port: PORT, username: USER, password: PASSWORD, readyTimeout: 25000 }); }); }
  ...
  return new Promise((resolve) => {
    sftp.mkdir(dir, { mode: 0o755 }, async (err) => {
      ...
      if (err.code === 2 || /No such file/i.test(err.message)) {
        ...
        if (parent && parent !== dir) { await sftpMkdirP(sftp, parent); return sftpMkdirP(sftp, dir).then(resolve); }
      }
      resolve();
    });
  });
}
function sftpUpload(sftp, local, remote) { return new Promise((resolve, reject) => sftp.fastPut(local, remote, (err) => err ? reject(err) : resolve())); }
function runExec(c, cmd) { return new Promise((resolve, reject) => { c.exec(cmd, (err, stream) => { if (err) return reject(err); let out = '', errOut = ''; stream.on('close', code => resolve({ code, out, errOut })); stream.on('data', d => out += d.toString()); stream.stderr.on('data', d => errOut += d.toString()); }); }); }
...
(async () => {
  ...
  if (!rootStat) { console.error(`✗ 远端路径 ${REMOTE_ROOT} 不存在`); c.end(); process.exit(2); }
  ...
  for (const rel of FILES) {
    ...
    catch (e) { console.error(`  ✗ 上传失败 ${rel}: ${e.message}`); }
  }
  ...
  const r2 = await runExec(c, `pm2 jlist 2>/dev/null | node -e "let d='';process.stdin.on('data',c=>d+=c).on('end',()=>{try{const j=JSON.parse(d).find(x=>x.name==='${PM2_APP}');if(j){console.log('status:',j.pm2_env.status,'restarts:',j.pm2_env.restart_time)}else console.log('(not found)')}catch(e){console.log('(parse err)',e.message)}})" 2>&1`);
