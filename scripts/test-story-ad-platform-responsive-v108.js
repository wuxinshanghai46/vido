...
const workspaceMarkup = `
  <main class="project-shell"><header class="platform-topbar project-topbar"><button class="wordmark">VIDO</button><b class="crumb">剧情广告</b><span class="crumb-separator">/</span><span class="project-title">多分辨率适配项目</span><span class="save-state">已保存</span><div class="top-actions"><button class="btn">任务中心</button><button class="btn">工作台</button><button class="icon-btn">⚙</button></div></header>
  <aside class="workspace-sidebar"><div class="side-label">剧情广告制作</div><nav>${['对话立项','剧情与对白','人物资产','场景世界','流向线稿','人物场景分镜','镜头与合成'].map((label,index)=>`<button class="workspace-nav${index===1?' active':''}"><span class="nav-number">${index+1}</span><span>${label}</span><small>${index===0?'完成':''}</small></button>`).join('')}<button class="workspace-nav workflow"><span class="nav-number">⌘</span><span>工作流画布</span></button></nav></aside>
...
async function geometry(page, marker) {
  return page.evaluate(marker => {
    ...
    return {
      ...
      sidebarBottom: document.querySelector('.workspace-sidebar')?.getBoundingClientRect().bottom || 0,
      mainLeft: document.querySelector('.workspace-main')?.getBoundingClientRect().left || 0,
      mainTop: document.querySelector('.workspace-main')?.getBoundingClientRect().top || 0,
  ...
  try {
    ...
    for (const [width, height] of viewports) {
      ...
      for (const [name, markup] of [['任务中心', projectCenterMarkup], ['六步工作区', workspaceMarkup]]) {
        ...
        if (name === '六步工作区') {
          if (width <= 900) assert.ok(result.sidebarBottom <= result.mainTop + 1, `${label} 移动端步骤导航不得覆盖内容区：${JSON.stringify(result)}`);
          else assert.ok(Math.abs(result.sidebarRight - result.mainLeft) <= 1, `${label} 侧栏与内容区不得重叠或留缝`);
          checks += 1;
