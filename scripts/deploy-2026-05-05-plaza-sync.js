/**
 * Deploy 2026-05-05d：广场删 female-med-1 + 用户 AI 形象自动同步到广场
 */
const fs = require('fs');
...
(async () => {
  ...
  try {
    ...
    // 删除 female-med-1 预设图片
    const rm = await exec(c, `find '${PRESETS_DIR}' -name 'avatar_female-med-1.*' -delete -print 2>/dev/null; echo ok`);
    const removed = rm.out.replace('ok', '').trim();
    if (removed) console.log(`[deploy] rm ${removed}`);
    console.log('[deploy] ✓ female-med-1 图片清理完成');

    await exec(c, `pm2 reload ${PM2_APP} --update-env 2>&1 || pm2 restart ${PM2_APP} 2>&1`);
    console.log('[deploy] ✓ pm2 reload');
    await new Promise(r => setTimeout(r, 2000));

    const pm2 = await exec(c, `pm2 jlist 2>/dev/null | node -e "let d='';process.stdin.on('data',c=>d+=c).on('end',()=>{const p=JSON.parse(d).find(x=>x.name==='${PM2_APP}');console.log('status='+p.pm2_env.status);})"`);
