...
function main() {
  ...
  const materialReferenceLazyJsFiles = allJsFiles.filter(file => /assetCenterMaterialReference/.test(file));
  const sceneQaProjectionLazyJsFiles = allJsFiles.filter(file => /sceneQaPublicState/.test(file));
  ...
  const sceneWorldLayoutLazyJsFiles = allJsFiles.filter(file => /sceneWorldLayoutViewer/.test(file));
  ...
  const storyStageLazyJsFiles = allJsFiles.filter(file => /(?:storyFlowSketchView|storyboardView)/.test(file));
  ...
  const materialReferenceLazyJsBytes = materialReferenceLazyJsFiles.reduce((sum, file) => sum + sourceBytes(file), 0);
  const sceneQaProjectionLazyJsBytes = sceneQaProjectionLazyJsFiles.reduce((sum, file) => sum + sourceBytes(file), 0);
  ...
  const briefPageLazyJsBytes = briefPageLazyJsFiles.reduce((sum, file) => sum + sourceBytes(file), 0);
  ...
  const sceneWorldLayoutLazyJsBytes = sceneWorldLayoutLazyJsFiles.reduce((sum, file) => sum + sourceBytes(file), 0);
  ...
  const storyStageLazyJsBytes = storyStageLazyJsFiles.reduce((sum, file) => sum + sourceBytes(file), 0);
  const shotDesignerLazyJsBytes = shotDesignerLazyJsFiles.reduce((sum, file) => sum + sourceBytes(file), 0);
  ...
  const materialReferenceLazyJsGzip = gzipBytes(materialReferenceLazyJsFiles);
  const sceneQaProjectionLazyJsGzip = gzipBytes(sceneQaProjectionLazyJsFiles);
  ...
  const briefPageLazyJsGzip = gzipBytes(briefPageLazyJsFiles);
  const panoramaLazyJsGzip = gzipBytes(panoramaLazyJsFiles);
  const sceneWorldLazyJsGzip = gzipBytes(sceneWorldLazyJsFiles);
  const sceneWorldLayoutLazyJsGzip = gzipBytes(sceneWorldLayoutLazyJsFiles);
  ...
  const storyStageLazyJsGzip = gzipBytes(storyStageLazyJsFiles);
  const shotDesignerLazyJsGzip = gzipBytes(shotDesignerLazyJsFiles);
  ...
  assert(dialogueDataJsBytes <= 8 * 1024, `对话数据与请求模块 ${dialogueDataJsBytes} bytes 超过 8 KiB`);
  assert(dialogueDataJsGzip <= 3 * 1024, `对话数据与请求模块 gzip ${dialogueDataJsGzip} bytes 超过 3 KiB`);
  ...
  assert(storyStageLazyJsBytes <= 48 * 1024, `流向线稿与人物场景分镜按需模块 ${storyStageLazyJsBytes} bytes 超过 48 KiB`);
  assert(storyStageLazyJsGzip <= 14 * 1024, `流向线稿与人物场景分镜按需模块 gzip ${storyStageLazyJsGzip} bytes 超过 14 KiB`);
  assert(shotDesignerLazyJsBytes <= 32 * 1024, `镜头设计器二级按需模块 ${shotDesignerLazyJsBytes} bytes 超过 32 KiB`);
  assert(shotDesignerLazyJsGzip <= 10 * 1024, `镜头设计器二级按需模块 gzip ${shotDesignerLazyJsGzip} bytes 超过 10 KiB`);
  ...
  assert(briefPageLazyJsBytes <= 42 * 1024, `对话立项目标页按需模块 ${briefPageLazyJsBytes} bytes 超过 42 KiB`);
  assert(briefPageLazyJsGzip <= 14 * 1024, `对话立项目标页按需模块 gzip ${briefPageLazyJsGzip} bytes 超过 14 KiB`);
  ...
  assert(sceneWorldLayoutLazyJsBytes <= 8 * 1024, `场景布局叠加按需模块 ${sceneWorldLayoutLazyJsBytes} bytes 超过 8 KiB`);
  assert(sceneWorldLayoutLazyJsGzip <= 4 * 1024, `场景布局叠加按需模块 gzip ${sceneWorldLayoutLazyJsGzip} bytes 超过 4 KiB`);
  assert(sceneWorldAuthorityLazyJsBytes <= 6 * 1024, `场景人物机位说明按需模块 ${sceneWorldAuthorityLazyJsBytes} bytes 超过 6 KiB`);
  assert(sceneWorldAuthorityLazyJsGzip <= 3 * 1024, `场景人物机位说明按需模块 gzip ${sceneWorldAuthorityLazyJsGzip} bytes 超过 3 KiB`);
  ...
  assert(soundDesignFeatureLazyJsGzip <= 12 * 1024, `声音设计功能模块 gzip ${soundDesignFeatureLazyJsGzip} bytes 超过 12 KiB`);
  assert(soundDesignLazyJsBytes + soundDesignFeatureLazyJsBytes <= 40 * 1024, '声音设计按需入口与功能模块合计超过 40 KiB');
  ...
  assert(materialReferenceLazyJsBytes <= 5 * 1024, `材料证据说明按需模块 ${materialReferenceLazyJsBytes} bytes 超过 5 KiB`);
  assert(materialReferenceLazyJsGzip <= 3 * 1024, `材料证据说明按需模块 gzip ${materialReferenceLazyJsGzip} bytes 超过 3 KiB`);
  assert(sceneQaProjectionLazyJsBytes <= 5 * 1024, `场景 QA 公开投影模块 ${sceneQaProjectionLazyJsBytes} bytes 超过 5 KiB`);
  assert(sceneQaProjectionLazyJsGzip <= 3 * 1024, `场景 QA 公开投影模块 gzip ${sceneQaProjectionLazyJsGzip} bytes 超过 3 KiB`);
