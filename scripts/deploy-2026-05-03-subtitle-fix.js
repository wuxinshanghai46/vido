// 部署 2026-05-03 数字人字幕/水印四 bug 修复：
// - effectsService.js：字幕时间戳按真实视频时长缩放，末段强制延伸至视频末尾（修复 Bug1/3/4）
// - digitalHuman.js：/api/dh/videos/generate 默认开启 coverWatermark（修复 Bug2）
const fs = require('fs');
...
(async () => {
  ...
  try {
    ...
    // 远端语法 check（Node 16 也兼容 require 顶层 try）
    for (const rel of files) {
      const remote = posix.join(REMOTE_ROOT, rel.replace(/\\/g, '/'));
