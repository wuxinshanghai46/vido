...
## 14. 2026-08-04 17:17 补充：重新识别反馈延迟与状态同步中断 v16

- 生产根因已闭环：终态同步把广告目标中的换行和空格误判为内容变化，轮询期间约每 35 秒再次执行项目更新，反复失效 16 个下游产物；当时启用的 SQLite + JSON 双写又使每个删除重写约 45MB 旧镜像，造成事件循环拥堵、接口延迟和页面 55% 同步中断。
- 客户端点击现在先本地显示 `queued / 1% / 重新识别请求已提交`，不再等 POST 才反馈；请求失败会恢复原权威状态。服务端对已同步终态保持只读，并用稳定终态时间、空白归一化和投影指纹保证幂等。
- 下游失效改为 SQLite 批处理；生产已固定为 SQLite 主读写，关闭 JSON 双写与 JSON 回退。旧 JSON 文件继续保留作为历史镜像，没有删除或覆盖现有业务数据。
- 提交 `2857c6b` 已推送；生产版本 `20260804-reference-sync-idempotency-v16 / reference-director-v2`，90/90 文件 SHA-256 一致。
- 目标分析现为 `completed / 100%`，语义质量有效，8/8 证据批次与 30 帧证据完整；完成后没有新增模型调用，任务内容版本连续三次稳定为 26，不再重放进度或重复失效下游。
- 本地与服务器完整回归均退出码 0；发布前后活动生成任务 0，PM2 online，内外网健康、数据库和 SQLite `quick_check` 正常。本地开发服务已加载 v16，地址 `http://localhost:3007`。
