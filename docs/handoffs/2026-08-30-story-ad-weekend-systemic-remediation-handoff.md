...
这三天不是只修复一个家居广告样例，而是把“场景生成 → 场景验收 → 系统内部剧情流向合同 → 人物场景分镜 → 视频生成前门禁”按全行业通用合同重新收敛。用户连续确认的关键要求如下：
...
5. 中间方案曾把“流向线稿”和“人物场景分镜”拆为不同业务产物；最终合同取消用户可见的流向线稿图片，由系统内部以零费用 `story_flow_contract` 完成人物/场景绑定，`/flow-sketches` 旧入口无条件返回 410。
...
差异基线为 `84be1d805e8b69b2347555302c864639e4ec51be`；首个纳入提交为 `5ebe3815348f686b90df42aa1d35eb8e868bf4a1`（2026-08-28 18:00:42）；结束应用构建提交为 `19e68a43bf7e9257a7c3b2a3e7c109df5691c61f`：

| 日期 | 提交数 | 主要内容 |
|---|---:|---|
| 2026-08-28 晚 | 25 | 场景失败恢复、QA 状态分流、页面加载截止时间、统一批处理入口、持久化进度 |
| 2026-08-29 | 101 | 媒体模型选择、场景并发与逐卡进度、真实空间布局、审核自动闭环、旧链路禁用、剧情流向合同演进、声音链路 |
...
- 多个场景由统一按钮启动服务端持久化批处理。V256 仍按场景顺序执行；独立并发 lane 和任务级聚合是在 8 月 29 日 V270 才完成，避免把周五的顺序批处理误当成最终并发合同。
...
- 最终图片目录：`Image · SZ/WB/DY`、`Nano Banana · SZ/WB/DY`，默认 `Image · SZ`；视频目录：`Seedance · DY/SZ/WB`，默认 `Seedance · DY`。
- 每次付费生成只调用用户选中的一条路线；首次付费调用失败立即停止，不自动切换供应商，避免重复计费。

#### 场景批量生成与真实进度

- 场景卡立即显示排队、提交、生成、审核、修复和完成状态；进度来自服务端持久化，不再依赖浏览器假计时。
- 人物和场景使用独立并发通道；不同场景可并行，同一目标不会重复许可。
- checkpoint 投影只把当前 attempt 的有效成功图片纳入当前权威集合；本次失败 attempt 不会覆盖权威结果，已经正式持久化的付费成功图片也不会因为一次 repair 失败被删除，只有内容、lineage 或 QA 策略变化才标记 stale。
- 修复 `repair checkpoint` 删除真实布局图后触发假空间兜底的问题；场景世界统一使用真实 master/layout/reverse/interaction/detail 资产。

#### 场景 QA 和旧链路禁用

- QA 结果支持部分成功，不再因为一个检查失败丢弃整份结果。只有取得可靠且可定位到具体视图的证据时，系统才最多执行一次有上限的局部修复并自动复核；无可靠证据时停止且图片调用为 0，审核不可用不能冒充图片质量失败。
...
- 中间版本先将六步流程拆成七步，区分流向线稿与人物场景分镜；随后按用户最终决定恢复六步界面，并彻底停用用户可见的流向线稿图片。
- 当前唯一新链路是零费用结构化 `story_flow_contract` 和系统自动人物/场景绑定；`storyFlowSketchService` 为禁用壳，旧 `/flow-sketches` 返回不可重试 410，不能触发图片供应商。人物场景分镜才生成实际图片。
...
- 增加项目级 `activeSketchBatches` 批次锁；同项目重复提交返回 HTTP 409 / `SKETCH_BATCH_IN_PROGRESS`，前端继续轮询已有持久化批次进度。`target_indexes` 只负责选择镜头，不是独立锁。
...
- V318 根据 `story_seed` 得到的当前声明/草稿顺序是：`现代高端家居展示厅 → 高端商业展台`。已持久化正式合同仍记录“展台 → 家居 → 展台”，但 story flow gate=false，旧合同被禁止复用；只有显式重新绑定后，正确顺序才会写入正式合同。
...
- V310 历史合同曾把家居 SH05/SH06 从极端材质特写修正为人物路线互动镜和完整空间建立镜，并同步清理目的、视觉字段和转场依据。V318 要求整体重新绑定，镜头数量、场景归属和编号可能变化，不能假设后续 SH05/SH06 仍承担相同角色。

#### 全行业空间与主体合同

- 新增通用场景域合同，按环境类型、主体类型、主体数量、运动方式、空间拓扑、决定性单帧和机位连续性编译。
- 合同编译器设计支持室内、户外、道路、车辆、动物群体、工业、轨道、空中、水域、舞台、桌面、抽象和虚拟空间；不是按行业关键词建立例外表。
- 当前确定性夹具明确覆盖 8 类：室内、户外、道路、动物、抽象、工业、空中、水域。车辆追逐、轨道、舞台、桌面和虚拟空间仍需专项视觉质量回归，不能写成已经完成同等强度验收。
...
```text
V283 → V284 → V288 → V289g → V290c → V291e
→ V292 → V299 → V301 → V305 → V309 → V310
→ V313 → V316 → V318
```

版本较多是因为发布门禁和生产 dry-run 持续暴露更早的数据根因，候选没有被强行上线：

- V285 候选因发布门禁/模块边界问题停止，拆分与迁移旧夹具后继续。
- V302 初次 dry-run 发现会把历史图片错误全量失效，停止应用并修复 lineage 兼容。
- V303 候选因历史 lineage 兼容回归失败停止，没有切换生产。
- V317 生产只读审计发现结尾“家居与展台”被误判为第三次场景回访，继续修复到 V318 才完成最终切换。

明天不得恢复 V285、V302 初版、V303 或 V317 的中间合同；当前唯一生产权威是 V318。
...
→ 场景 QA / 自动局部修复 / 用户接受
→ 零费用 story_flow_contract 与系统自动绑定
...
- `public/story-ad/views/storyFlowSketchView.js`：历史七步阶段前端模块；当前主链路已禁用流向线稿图片，不应恢复使用。
...
- 目标任务包含 6 条历史 unknown 计费证据：`storyboard_image` 1 条、`storyboard_rewrite` 3 条、`storyboard_table` 1 条、`scene_extension_detail` 1 条；当前活动 unknown 为 0。不得把“非活动”理解为“可以自动重试”。
...
- 批量 `readDb()` 审计脚本因结果过大触发 Python sqlite bridge `ENOBUFS`；没有用失败结果冒充通过。清单逐文件校验、专用活动任务脚本和 SQLite quick check 分别完成了发布完整性与运行安全核对，但不能替代全库业务语义审计；大范围业务数据审计本轮未完成。
...
5. 若决定重新生成分镜，不能只确认历史 unknown 已非活动；必须先完成供应商侧账单核对，或由用户明确接受潜在重复计费风险并形成生成许可记录。之后只对明确目标镜头提交，观察总进度、逐镜进度、任务锁和失败恢复。
