...
2026-07-28 UI 层级微调：唯一的“下一步：编写剧情与表演”主按钮已移动到“基础信息 / 场景配置”标题栏右上角；场景资产结果下方只保留就绪状态摘要。按钮仍使用同一资产就绪判断，未完成时禁用，进入剧情设置后隐藏，服务端 `story_setup_confirmed` 门禁没有放宽。资产就绪后按钮同时取得与其它“下一步”主操作一致的 `is-next` 状态，统一渐变背景、文字颜色、悬停位移和阴影；禁用或忙碌时不会显示可点击指向。
...
- `public/js/new-story-ad/bootstrap.js`
  - 前端缓存版本 `20260728-story-setup-cta-v38`。
