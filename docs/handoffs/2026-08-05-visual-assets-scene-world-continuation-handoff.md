...
- 联合任务已于 00:58 进入 `visual_assets_failed`，支持编号 `8e4a6485-2fea-4133-839d-eeea67e3b868`。
- 最新失败发生在 `new_story_ad.person_dossier_wardrobe_detail`，模型 `deyunai/gpt-image-2` 返回 HTTP 500，供应商内部原因 `UNKXXXO004IFR`；平台分类为 `PROVIDER_5XX_AMBIGUOUS`。供应商没有公开该内部码，因此不能断言是审核拦截还是服务故障。
- 失败请求没有 provider task ID，状态为 `submitted_unknown / billing unknown`，禁止自动重试。
- 六个核心人物单元 `body / identity / expression / action / face_master / body_master` 均已成功且计费确认；本轮另有两次配饰细节调用成功。最后的服装细节失败发生在人物资产最终组装之前，因此外层人物检查点为 failed，完整人物资产尚未提交。
- “人物 6/6 后仍失败”存在进度口径缺陷：6/6 未包含后续必做的配饰、服装细节和档案合成。该缺陷会放大一次补充图失败的影响，明天应优先修复进度总量、可选细节降级边界及阶段性人物资产提交。
- 平台记录中场景生成检查点为 0，未发现本轮场景模型提交；场景 0/1 不是内容审核结果，而是联合任务在人物分支失败时没有形成场景结果。
