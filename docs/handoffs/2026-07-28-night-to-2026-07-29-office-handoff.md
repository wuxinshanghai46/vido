...
## 十五、11:22 第 3 步主按钮位置统一

- “生成剧本”已从剧情设置内容面板底部移到第 3 步标题栏右侧，与其他步骤的主操作保持同一位置。
- 已删除底部专用操作容器、上边框和额外内边距，避免按钮改变剧情设置内容区的格局。
- 新增回归断言：按钮必须位于第 3 步标题区并在内容面板之前，页面不得残留底部操作容器。
- 浏览器缓存版本提升到 `20260728-story-step-v42`。
- 本地针对性检查与完整 `story-ad:v3:test` 均通过；本地浏览器核对按钮唯一、属于第 3 步顶部操作容器，且位于内容面板之前。
- 本轮未点击 AI 辅写或生成剧本，未触发文本、图片、视频模型或任务写入。

## 十六、11:42 刷新后长文本分段丢失修复

- 根因：广告需求与剧情表演正文在前端原本带有真实换行，但自动保存进入服务端 `buildContext` 后被通用单行清洗折叠为空格；刷新只能读到已压平的持久化内容。
- 服务端新增多行文本清洗，广告需求和剧情表演 `raw` 保存时保留换行与空行，仅规范每行内部多余空格。
- 刷新恢复端兼容历史压平任务，会按 `【栏目】`、数字编号和 `•` 项目符号重新分段；用户再次保存后将以多行格式持久化。
- 新增真实 `createTask/updateTaskRequest/context` 持久化回归，验证保存后的两段长文本与输入字符串逐字一致。
- 浏览器缓存版本提升到 `20260728-story-step-v43`。
- 本地定向回归、语法检查与完整 `story-ad:v3:test` 均通过；未调用真实文本、图片或视频模型。
