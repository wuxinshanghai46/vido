...
## 10. 2026-08-10 v136 年龄、画面形态与剧本结构升级（最新）

### 已完成内容

- 人物年龄改为独立结构化字段，支持确定年龄和年龄区间；如 `18岁`、`18~25岁`、`18～25岁`、`18-25岁`、`18至25岁` 均会规范化保存，并作为人物身份、人物档案、图片和后续镜头的硬约束。
- 修复年龄保存的两个根因：
  1. 后端不再从外貌文本无条件删除年龄，也不会把 `100岁`、`1000岁` 截断；
  2. 人物保存成功后只刷新 `summary,assets` 并重挂当前资产视图，避免保存成功但页面继续显示旧闭包数据。
- 保存按钮固定在人物抽屉底部；中间表单独立滚动，长内容和多造型不再被底部区域遮挡。
- 项目级画面形态新增：自动识别、真人实拍、电影级 3D、二维动漫、动态漫、混合媒介、自定义。画面形态从项目世界设定传递到人物、动物、场景、关键帧和人物档案；旧真人档案在画面形态变化后会标记失效，禁止继续复用。
- “国家/地区”和“具体时期”留空时，会从当前剧本、内容目标和已确认参考自动分析；用户填写时以用户输入为最高权威，不按行业写死。
- AI 帮写输出统一为正常剧本结构，剧情和广告均包含：原始要求、详细剧情梗概、出场人物、场景清单、分段剧情/场次正文、结尾落点；广告额外保留品牌/商品/服务证明和行动引导，但不会把镜头级分镜提前混入内容目标。
- 多造型仍保持“一人一张人物卡、每套造型独立视觉档案”的数据结构；保存时保留结构化造型合同。分镜和关键帧的多人多造型显式展示/绑定属于后续独立增强，不在本次 v136 付费生成前强行扩面。
- 提取 `sceneVisualPromptService.js`，人物与场景提示按画面形态复用公共合同；没有增加行业分支，知识正文不进入首屏 bundle。

### Git 与发布身份

- 分支：`codex/story-ad-v3-upgrade`
- 功能提交：`7eb4507 feat(story-ad): lock age medium and screenplay structure`
- 回归提交：`27c509f test(story-ad): cover age and visual medium contracts`
- Build：`20260810-age-medium-script-v136`
- Artifact：`61eaca203de6bcc4f904c34202d09d0db4f935dec6713a743e0df66d894dffd6`
- Source snapshot：`95a69e2b8d886138f871b4328003b3d9b44896f3ba22d7a62db9f00dbf5521f5`
- Release bundle：`d49b69c92b34549c71f723acfb9bf9951de3bfa5840e02c6215b50bfbe3edce1`
- Runtime hash：`bc68635608e19ec751478af4e23811179c0295ebecee1d6c972cbf54a24a2e50`
- 生产 release：`/opt/vido/releases/61eaca203de6bcc4f904c34202d09d0db4f935dec6713a743e0df66d894dffd6`

### 三方一致性结论

| 核对项 | 本地 | origin/Gitee | 生产 | 结论 |
|---|---|---|---|---|
| v136 功能与测试代码 | `7eb4507` + `27c509f` | 已推送 | 651 文件不可变发布包 | 一致 |
| Build | `20260810-age-medium-script-v136` | 发布清单已提交 | 同值 | 一致 |
| Artifact | `61eaca...ffd6` | 发布清单已提交 | 同值 | 一致 |
| Source snapshot | `95a69e...21f5` | 发布清单已提交 | 同值 | 一致 |
| Bundle | `d49b69...dce1` | 发布清单已提交 | 同值 | 一致 |
| Runtime | `bc6863...2e50` | 发布清单已提交 | 同值 | 一致 |
| 旧 UI | 本地关闭 | 关闭逻辑已提交 | `legacy_story_ad_ui_enabled=false` | 一致 |

### 实际验证

- `npm run platform:upgrade:test`：退出码 0，用时 327.7 秒；35 页面加载、120 镜性能、知识策略、人物/场景/分镜/关键帧/视频/音频/可靠性链路通过。
- 知识策略：`industry_hardcoded_branches=0`，编译模型调用 0。
- 年龄确定值/区间、保存后 read-back、多造型 25 项、人物状态失效、真人/3D/动漫媒介传递、标准剧本结构均通过定向回归。
- 场景空间回归：56 次模拟生成，峰值并发 3，模型管理只允许 Image2，真实付费调用 0。
- `story-ad:release:test`：完整性 11 项、传输 7 项、原子构建、651 文件闭包均通过。
- 生产部署：发布前后活动任务 `0 -> 0`，未知计费 `0 -> 0`，迁移模型调用 0、付费调用 0，SQLite `quick_check=ok`。
- 生产 PM2：`vido` PID 25922；公网与服务器内网版本接口均返回 v136，health 为 `ok`。
- 本地与生产版本接口的 build/artifact/source snapshot/bundle/runtime 五项值完全一致。

### 未执行项与验收边界

- 未执行真实图片、视频、360 或 6DoF 付费端到端生成，避免在用户验收前产生费用。
- 未替用户修改目标任务或点击重试；旧任务需要用户进入 v136 后先确认人物年龄、项目画面形态和世界设定，再由用户显式触发。
- 生产部署和代码门禁已完成；下一步是用户进行页面业务验收，重点检查年龄保存刷新、长抽屉滚动、项目画面形态、自动时期/地区提示、AI 帮写的剧本格式。
