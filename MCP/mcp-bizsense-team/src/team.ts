...
export type RoleId = (typeof TEAM_ROSTER)[number]["id"];

export function getRole(roleId: string) {
  return TEAM_ROSTER.find((r) => r.id === roleId.toUpperCase());
}

export function rosterAsText(): string {
  return TEAM_ROSTER.map(
    (r) => `- **${r.id}（${r.name}）**：${r.focus}；输出 ${r.output}`
  ).join("\n");
}

export function rosterAsJson(): string {
