...
function withRead(collection, legacyFn, selectFn) {
  if (!isSqliteEnabled() || !shouldReadPrimary()) return legacyFn();
  const value = selectFn(records.list(collection));
...
function adapt(legacy) {
  return {
    ...
    listProjects(userId) { return withRead('projects', () => legacy.listProjects(userId), rows => descCreated(rows.filter(p => !userId || p.user_id === userId))); },
    ...
    getStoryByProject(projectId) { return withRead('stories', () => legacy.getStoryByProject(projectId), rows => rows.find(s => s.project_id === projectId) || null); },
    ...
    getClipsByProject(projectId) {
      return withRead('video_clips', () => legacy.getClipsByProject(projectId), rows => rows.filter(c => c.project_id === projectId).sort((a, b) => (a.scene_index || 0) - (b.scene_index || 0)));
    },
    getClip(id, projectId) { return withRead('video_clips', () => legacy.getClip(id, projectId), rows => rows.find(c => c.id === id && c.project_id === projectId) || null); },
    ...
    listI2VTasks(userId) { return withRead('i2v_tasks', () => legacy.listI2VTasks(userId), rows => descCreated(rows.filter(t => !userId || t.user_id === userId))); },
    ...
    listNovels(userId) { return withRead('novels', () => legacy.listNovels(userId), rows => descCreated(rows.filter(n => !userId || n.user_id === userId))); },
    ...
    listAssets(userId, type) { return withRead('assets', () => legacy.listAssets(userId, type), rows => descCreated(rows.filter(a => (!userId || a.user_id === userId) && (!type || type === 'all' || a.type === type)))); },
    ...
    listPortraits(userId) { return withRead('portraits', () => legacy.listPortraits(userId), rows => descCreated(rows.filter(t => !userId || t.user_id === userId))); },
    ...
    listComicTasks(userId) { return withRead('comic_tasks', () => legacy.listComicTasks(userId), rows => descCreated(rows.filter(t => !userId || t.user_id === userId))); },
    ...
    listAvatarTasks(userId) { return withRead('avatar_tasks', () => legacy.listAvatarTasks(userId), rows => descCreated(rows.filter(t => !userId || t.user_id === userId))); },
    ...
    listMonitors(userId) { return withRead('accounts', () => legacy.listMonitors(userId), rows => descCreated(rows.filter(m => !userId || m.user_id === userId))); },
    ...
    listContents(userId, accountId) { return withRead('contents', () => legacy.listContents(userId, accountId), rows => descCreated(rows.filter(c => (!userId || c.user_id === userId) && (!accountId || c.account_id === accountId)))); },
    ...
    listSubscriptions(userId) { return withRead('subscriptions', () => legacy.listSubscriptions(userId), rows => descCreated(rows.filter(s => !userId || s.user_id === userId))); },
    ...
    listReplicateTasks(userId) { return withRead('tasks', () => legacy.listReplicateTasks(userId), rows => descCreated(rows.filter(t => !userId || t.user_id === userId))); },
    ...
    listVoices(userId) { return withRead('voices', () => legacy.listVoices(userId), rows => descCreated(rows.filter(v => !userId || v.user_id === userId))); },
    ...
    listAIChars(userId) { return withRead('characters', () => legacy.listAIChars(userId), rows => descCreated(rows.filter(c => !userId || c.user_id === userId))); },
    ...
    listAIScenes(userId) { return withRead('scenes', () => legacy.listAIScenes(userId), rows => descCreated(rows.filter(s => !userId || s.user_id === userId))); },
    ...
    listDramaProjects(userId) { return withRead('drama_projects', () => legacy.listDramaProjects(userId), rows => descCreated(rows.filter(r => !userId || r.user_id === userId))); },
    ...
    listDramaEpisodes(projectId) { return withRead('drama_episodes', () => legacy.listDramaEpisodes(projectId), rows => rows.filter(r => r.project_id === projectId).sort((a, b) => (a.episode_index || 0) - (b.episode_index || 0))); },
    ...
    listDramaTasks(userId) { return withRead('drama_episodes', () => legacy.listDramaTasks(userId), rows => descCreated(rows.filter(r => !userId || r.user_id === userId))); },
