...
const dossierComposites = require('../services/newStoryAd/dossierCompositeService');
const pipelineModels = require('../services/pipelineModelService');
...
async function runUnifiedVisualAssetLanes({ taskId, body, job, userId }) {
  ...
  const sceneLane = (async () => {
    ...
    for (let index = 0; index < sceneTargets.length; index += 1) {
      ...
      try {
        ...
        sceneFailures.push({ scene_id: target.scene_id, scene_name: target.name, error: sceneError });
    ...
    if (sceneFailures.length) {
      const primary = sceneFailures.find(item => item.error?.billingState === 'unknown' || item.error?.billing_state === 'unknown'
      ...
      error.scene_failures = sceneFailures.map(item => ({ scene_id: item.scene_id, scene_name: item.scene_name,
...
function productionImagePricePlan() {
  ...
  const maximumUnitUsd = priced.length && !unknown.length ? Math.max(...priced.map(item => item.unit_price_usd)) : NaN;
  return { routes: priced, unknown, maximum_unit_usd: maximumUnitUsd };
}

function unifiedProductionPlan(taskId, body = {}) {
  ...
  const wardrobeDetailCalls = castProfiles.reduce((sum, profile) => sum + 4 + dossierComposites.explicitAccessoryDefinitions(profile).length, 0);
  const basis = {
    contract_version: productionGraph.CONTRACT_VERSION,
    task_id: taskId,
    content_revision: Number(task.content_revision || 1) || 1,
    people, animals, scene_count: sceneCount, panorama_count: panoramaCount,
    estimated_model_calls: {
      text_planning_max: people + 3,
      person_dossier_max: people * 6 + wardrobeDetailCalls,
      animal_dossier_max: animals,
      prop_assets_max: carriedProps.reduce((sum, prop) => sum + (Array.isArray(prop.states) && prop.states.length > 1 ? 2 : 1), 0),
      scene_assets_max: sceneCount * 5,
      panorama_image_max: panoramaCount,
      panorama_qa_text_max: panoramaCount,
    },
  ...
  const estimatedVisualCostRmb = Number.isFinite(pricePlan.maximum_unit_usd)
    ? Number((imageCalls * pricePlan.maximum_unit_usd * Number(tokenTracker.getUSDtoCNY() || 7.2)).toFixed(2)) : null;
  const pricedBasis = { ...basis, estimated_paid_image_calls_max: imageCalls,
    estimated_visual_cost_max_rmb: estimatedVisualCostRmb,
  ...
  return { ...pricedBasis, plan_fingerprint: productionGraph.fingerprint(pricedBasis) };
}

function assertUnifiedProductionConfirmation(body = {}, expected = {}) {
  ...
  error.code = 'PRODUCTION_GRAPH_COST_CONFIRMATION_REQUIRED'; error.status = 400; error.retryable = false; error.current_plan = expected;
  throw error;
}

async function runUnifiedProductionAssets({ taskId, body, job, userId, user }) {
  ...
  try {
    ...
    const executionPlan = unifiedProductionPlan(taskId, body);
    assertUnifiedProductionConfirmation({ ...body, plan_fingerprint: executionPlan.plan_fingerprint }, executionPlan);
    ...
    const personBody = registerPersonPlanGenerationRoute.currentPersonGenerationBody({ taskId, input: body, service, storage });
    ...
    const sceneTargets = (scenePlan.spaces || scenePlan.scenes || []).map((space, index) => ({
      scene_id: String(space.id || space.space_id || space.scene_id || '').trim(),
      space_id: String(space.id || space.space_id || space.scene_id || '').trim(),
      name: String(space.name || `场景 ${index + 1}`).trim(),
      scene_spec: space.scene_spec || space.sceneSpec || {},
    })).filter(item => item.scene_id);
    const visualBody = { ...personBody, ...body, task_id: taskId, generate_subjects: true, scene_targets: sceneTargets };
    const visualAssets = await runUnifiedVisualAssetLanes({ taskId, body: visualBody, job, userId });
    ...
    for (const prop of graph.props.filter(item => item.attachment_mode === 'carried' && !item.image_url)) {
      if (!prop.description) {
        const error = new Error(`随身物“${prop.name}”缺少可生成的完整外观、材质和用途描述。`);
    ...
    if (body.generate_panoramas !== false) {
      ...
      panoramas = await scenePanoramaService.generateTaskPanoramas(taskId, {
        plan_fingerprint: panoramaPlan.plan_fingerprint,
        cost_confirmation: true,
        generation_id: job.generationId,
      }, { generationId: job.generationId });
      if (panoramas.failed_count) {
        const error = new Error(`有 ${panoramas.failed_count} 个360场景未通过生成或质检；成功场景已保留。`);
        error.code = 'PRODUCTION_GRAPH_PANORAMA_PARTIAL_FAILED'; error.retryable = true; error.partial = panoramas;
        throw error;
      }
    }
    ...
    if (graph.validation.status !== 'ready') {
      const error = new Error(`统一制作图谱仍有 ${graph.validation.issues.length} 个合同缺项，已停止进入付费关键帧。`);
      error.code = 'PRODUCTION_GRAPH_INCOMPLETE'; error.retryable = true; error.details = graph.validation;
      throw error;
    }
    ...
    error.production_graph_status = graph?.validation?.status || 'unavailable';
    throw error;
  }
}

router.post('/tasks/:id/production-assets/plan', asyncRoute(async (req, res) => {
  taskForReq(req);
  res.setHeader('Cache-Control', 'private, no-store, no-cache, must-revalidate');
  res.json({ success: true, ...unifiedProductionPlan(req.params.id, req.body || {}) });
}));

router.post('/tasks/:id/production-assets', asyncRoute(async (req, res) => {
  taskForReq(req);
  const body = req.body || {};
  const expected = unifiedProductionPlan(req.params.id, body);
  const confirmedCostLimitRmb = assertUnifiedProductionConfirmation(body, expected);
  const user = userFromReq(req);
  ...
  return queueTaskStage(req, res, 'production_assets', job => runUnifiedProductionAssets({
...
router.post('/tasks/:id/visual-assets', asyncRoute(async (req, res) => {
  taskForReq(req);
  productionGraph.assertLegacyMutationAllowed(req.params.id, 'visual_assets');
  const user = userFromReq(req);
  const userId = String(user.id || user.userId || user.username || 'anonymous');
  const body = { ...(req.body || {}), task_id: req.params.id };
  const sceneTargets = visualAssetOrchestration.normalizedSceneTargets(body);
...
router.post('/tasks/:id/tts', asyncRoute(async (req, res) => {
  const body = { ...(req.body || {}), request_base_url: (process.env.PUBLIC_BASE_URL || `${req.protocol}://${req.get('host')}`).replace(/\/$/, '') };
  return queueTaskStage(
    req,
    res,
    'tts',
