...
const productionAssetOrchestratorFactory = require('../services/newStoryAd/productionAssetOrchestratorService');
...
const personDossiers = require('../services/newStoryAd/personDossierService'), propAssetService = require('../services/newStoryAd/propAssetService'), registerPropRoutes = require('./newStoryAd/propRoutes'), registerPersonPlanGenerationRoute = require('./newStoryAd/personPlanGenerationRoute'), registerSceneBatchRoutes = require('./newStoryAd/sceneBatchRoutes');
...
const productionAssets = productionAssetOrchestratorFactory.create({ service, storage, generateAndCommitSubjectAssets, persistProviderPersonIds });

router.post('/tasks/:id/production-assets/plan', asyncRoute(async (req, res) => {
  taskForReq(req);
  res.setHeader('Cache-Control', 'private, no-store, no-cache, must-revalidate');
  res.json({ success: true, ...productionAssets.plan(req.params.id, req.body || {}) });
}));
...
router.post('/tasks/:id/visual-assets', asyncRoute(async (req) => {
  taskForReq(req); productionGraph.assertLegacyMutationAllowed(req.params.id, 'visual_assets');
}));
