...
function _probeLocalMediaDuration(filePath, fallback = 0) {
  ...
  try {
    ...
    const out = execFileSync(ffmpegPath, ['-i', filePath], { encoding: 'utf8', timeout: 10000, stdio: ['ignore', 'pipe', 'pipe'] });
    const m = String(out || '').match(/Duration:\s*(\d+):(\d+):([\d.]+)/);
    return m ? (+m[1] * 3600 + +m[2] * 60 + +m[3]) : fallback;
  } catch (err) {
...
function _resolveLocalPublicAsset(urlOrPath) {
  ...
  if (i >= 0) {
    const name = path.basename(src.slice(i + marker.length).split('?')[0]);
    const p = path.join(__dirname, '../../outputs/jimeng-assets', name);
    if (fs.existsSync(p)) return p;
  }
  const avatarMarker = '/api/avatar/audios/';
  const j = src.indexOf(avatarMarker);
  if (j >= 0) {
    const name = path.basename(src.slice(j + avatarMarker.length).split('?')[0]);
...
router.post('/preview-voice', async (req, res) => {
  const { text, voiceId, gender, speed, providerId = '', provider = '' } = req.body;
  const sampleText = ((text || '').trim() || '你好，这是 VIDO 数字人配音试听。现在你听到的是当前选择的音色。').slice(0, 220);
  ...
  try {
    ...
    if (hitPath && fs.statSync(hitPath).size > 2048) {
    ...
    if (actualPath && fs.statSync(actualPath).size > 2048) {
      ...
      res.status(500).json({ success: false, error: 'TTS 文件生成失败或音频为空' });
    }
  } catch (err) {
...
// 会被跳过并打 warn 日志。
// ═══════════════════════════════════════════════
async function _dispatchLipSync({ imageUrl, audioUrl, maskUrls, prompt, baseUrl, taskId, onProgress, userId, agentId, strictModel = null, strictMode = false }) {
  ...
  const stageId = agentId || 'avatar.lip_sync';
  let chain = strictModel
    ? [{ provider_id: strictModel.provider_id, model_id: strictModel.model_id, priority: 1, enabled: true }]
    : pms.pickAllEnabled(stageId);
  let usingFallbackStage = false;
  if (!strictMode && (!chain || !chain.length) && stageId !== 'avatar.lip_sync') {
    chain = pms.pickAllEnabled('avatar.lip_sync');
    usingFallbackStage = true;
  }
  if (!chain || !chain.length) {
    throw new Error(`${stageId} 未启用任何模型：请先在管理后台「模型调用管理」选择并启用飞影/Topview/即梦等口型同步模型；系统不会自动回退到即梦。`);
  }
  ...
  try {
    ...
    if (!strictMode && (stageId === 'avatar.lip_sync' || usingFallbackStage) && hasTopviewAuth && topviewModel?.id && !chain.some(m => m.provider_id === 'topview' || m.model_id === topviewModel.id)) {
  ...
  console.log(`[lip-sync:dispatch] ${stageId} 用户配置链: ${chain.map(m => `${m.provider_id}/${m.model_id}`).join(' → ')}`);

  let lastError = null;
  let triedAny = false;
  for (const m of chain) {
    if (!LIP_SYNC_CAPABLE.has(m.model_id)) {
      const msg = `${m.provider_id}/${m.model_id} 不支持 image+audio 口型同步`;
      if (strictMode) throw new Error(`强制链路模型不可用：${msg}`);
      console.warn(`[lip-sync:dispatch] 跳过 ${msg}（仅文生视频）`);
      continue;
    }
    ...
    // 而不是等下游 service 抛"未配置"才知道）
    {
      ...
      if (_depProv && _depProv.enabled === false) {
        if (strictMode) throw new Error(`强制链路模型不可用：实际依赖的 provider "${actualPid}" 已禁用`);
    ...
    try {
      if (m.provider_id === 'topview' || String(m.model_id || '').startsWith('topview-avatar')) {
        const localAudio = _resolveLocalPublicAsset(audioUrl);
        const audioDuration = _probeLocalMediaDuration(localAudio, 0);
        const topviewLongAudioLimit = String(m.model_id || '').toLowerCase().includes('avatar4') ? 120 : 28;
        if (audioDuration > topviewLongAudioLimit) {
          const msg = `Topview ${m.model_id || 'video_avatar'} audio limit is about ${topviewLongAudioLimit}s, current audio ${audioDuration.toFixed(1)}s`;
          if (strictMode) throw new Error(`强制链路失败：${msg}`);
          console.warn(`[lip-sync:dispatch] ${msg}`);
          if (typeof onProgress === 'function') onProgress({ stage: 'fallback', message: msg, model_id: m.model_id });
      ...
      if (m.model_id === 'hifly' || m.model_id === 'hifly-free' || m.provider_id === 'hifly') {
        ...
        if (!_hiflyProv || !_hiflyProv.api_key) {
          if (strictMode) throw new Error('强制链路模型不可用：未配置 hifly api_key');
      ...
      if (m.model_id === 'wan2.2-animate-move') {
        if (strictMode) throw new Error('强制链路模型不可用：wan2.2-animate-move 需要模板视频，不支持当前 image+audio 输入');
        console.warn(`[lip-sync:dispatch] wan2.2-animate-move 需要模板视频（不接受单纯 audio），跳过`);
        continue;
      }
      // 其他能力暂未实现的模型一律跳过
      if (strictMode) throw new Error(`强制链路模型不可用：${m.provider_id}/${m.model_id} 尚未对接口型同步路由`);
  ...
  if (!triedAny) {
    throw new Error(`${stageId} 调度失败：当前启用链中没有任何"可口型同步"的模型（请在管理后台「模型调用管理」启用飞影/Topview/即梦等支持 image+audio 的模型）`);
  }
  if (lastError) throw lastError;
  throw new Error(`${stageId} 调度失败：所有候选模型均不可用`);
}
...
function _splitExpressiveText(text, maxChars = 42) {
  ...
  for (const part of rough) {
    ...
    while (s.length > maxChars) {
      let cut = Math.max(16, s.slice(0, maxChars).lastIndexOf('，'));
      if (cut < 16) cut = maxChars;
...
function _expressiveToneParams(i, total) {
  const curve = [
    { speed: 0.94, pitch: 0.98 },
    { speed: 1.02, pitch: 1.03 },
    { speed: 1.08, pitch: 1.07 },
    { speed: 0.98, pitch: 1.00 },
    { speed: 1.05, pitch: 1.05 },
  ];
  const picked = curve[i % curve.length];
  if (i === total - 1) return { speed: 0.96, pitch: 0.99 };
  return picked;
}

async function _generateExpressiveSpeech(text, outputBase, { voiceId = null, baseSpeed = 1.0 } = {}) {
  const parts = _splitExpressiveText(text);
  ...
  const ffmpegPath = (process.env.FFMPEG_PATH && process.env.FFMPEG_PATH !== 'ffmpeg')
    ...
    : require('ffmpeg-static');
  const { execFileSync } = require('child_process');
  const workDir = `${outputBase}_expressive`;
  fs.mkdirSync(workDir, { recursive: true });
  const files = [];
  for (let i = 0; i < parts.length; i++) {
    const p = _expressiveToneParams(i, parts.length);
    const segBase = path.join(workDir, `seg_${String(i).padStart(2, '0')}`);
    const segFile = await generateSpeech(parts[i], segBase, {
      voiceId,
      speed: Math.max(0.78, Math.min(1.22, (Number(baseSpeed) || 1.0) * p.speed)),
      pitch: p.pitch,
    });
    if (!segFile || !fs.existsSync(segFile)) throw new Error(`分段配音 ${i + 1} 生成失败`);
    files.push(segFile);
  }
  const listPath = path.join(workDir, 'concat.txt');
  fs.writeFileSync(listPath, files.map(f => `file '${f.replace(/\\/g, '/').replace(/'/g, "'\\''")}'`).join('\n'), 'utf8');
  const outPath = outputBase.replace(/\.[^.]+$/, '') + '.mp3';
  execFileSync(ffmpegPath, ['-y', '-f', 'concat', '-safe', '0', '-i', listPath, '-af', 'dynaudnorm=f=150:g=7,acompressor=threshold=-18dB:ratio=2.2:attack=12:release=180', '-c:a', 'libmp3lame', '-q:a', '3', outPath], { stdio: 'pipe', timeout: 120000 });
...
function _splitOmniSubtitleText(text, maxChars = 16) {
  const src = String(text || '').replace(/\s+/g, ' ').trim();
  if (!src) return [];
  ...
  for (const part of parts) {
    ...
    while (s.length > maxChars) {
      let cut = maxChars;
      const m = s.slice(0, maxChars + 8).match(/^.*[。！？!?；;，,、]/);
      if (m && m[0].length >= Math.floor(maxChars * 0.55)) cut = m[0].length;
      out.push(s.slice(0, cut).trim());
      s = s.slice(cut).trim();
    }
    if (s) out.push(s);
...
function _buildOmniSubtitleEffects({ text = '', segments = [], subtitle = {} }) {
  if (subtitle?.show === false) return [];
  const source = Array.isArray(segments) && segments.length
    ? segments
    : [{ text, start: 0, end: Math.max(1, String(text || '').length * 0.25) }];
  const style = subtitle?.style || 'popup';
  const position = subtitle?.position || (style === 'comic' ? 'top-center' : 'bottom-center');
  const effects = [];
  ...
  for (const seg of source) {
    ...
    const end = Number.isFinite(rawEnd) && rawEnd > start ? rawEnd : start + Math.max(0.8, segText.length * 0.25);
    const chunks = _splitOmniSubtitleText(segText, 14);
    const total = chunks.reduce((sum, chunk) => sum + Math.max(1, chunk.length), 0) || 1;
    let cursor = start;
    chunks.forEach((chunk, idx) => {
      const last = idx === chunks.length - 1;
      const dur = last ? end - cursor : Math.max(0.55, (end - start) * (Math.max(1, chunk.length) / total));
      const next = last ? end : Math.min(end - 0.05, cursor + dur);
      effects.push({
        text: chunk,
        position,
        style: 'subtitle',
        ...
        startTime: cursor,
        endTime: Math.max(cursor + 0.35, next),
        fontName: subtitle?.fontName || '抖音美好体',
        fontSize: Number(subtitle?.fontSize) || 60,
...
router.post('/jimeng-omni/generate', async (req, res) => {
  ...
  const strictMode = strict_mode === true || strict_mode === 'true';
  const effectiveTextEffects = Array.isArray(textEffects) && textEffects.length
    ? textEffects
    : _buildOmniSubtitleEffects({ text, segments, subtitle });
  ...
  const task = {
    ...
    post_effects: { textEffects: effectiveTextEffects, stickers: stickers || [], pointers: pointers || [], cameraMotion: cameraMotion || 'static', cameraSegments: req.body?.cameraSegments || [], coverWatermark: coverWatermark === true },
  };
  ...
  (async () => {
    try {
      ...
      if (audio_url) {
        ...
        let result = await _generateExpressiveSpeech(text, audioBase, { voiceId: voiceId || null, baseSpeed: Number(speed) || 1.0 });
        if (!result) result = await generateSpeech(text, audioBase, { voiceId: voiceId || null, speed: Number(speed) || 1.0 });
      ...
      const DEFAULT_OMNI_PROMPT = [
        ...
        agentId === 'ad_avatar.lip_sync'
          ? '广告导览动作要明显但真实：允许自然抬手、张开手掌介绍、指向展示区域、身体微微转向展墙再回看镜头；不要全程站立不动。'
