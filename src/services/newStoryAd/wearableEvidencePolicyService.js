...
async function resolve(options = {}, deps = {}) {
  ...
  const appearanceDefinitions = definitions.filter(definition => definition.key === 'hair_makeup');
  const isolatedObjectDefinitions = definitions.filter(definition => definition.key !== 'hair_makeup');
  const local = appearanceDefinitions.length
    ? await composites.composeWearableDetails({ ...options, definitions: appearanceDefinitions }, deps)
    : [];
  const critical = criticalDefinitions(options.profile || {}, definitions);
  const isolated = isolatedObjectDefinitions.length
    ? await composites.generateWearableDetails({ ...options, definitions: isolatedObjectDefinitions }, deps)
    : [];
  const localByKey = new Map(local.map(item => [item.key, { ...item, evidence_status: 'ready', evidence_mode: 'local_crop' }]));
  const isolatedByKey = new Map(isolated.map(item => [item.key, { ...item, evidence_status: 'ready', evidence_mode: 'isolated_catalog_generation' }]));
  const items = definitions.map(definition => (
    isolatedByKey.get(definition.key)
  ...
  return {
    ...
    trace: {
      ...
      isolated: items.filter(item => item.evidence_mode === 'isolated_catalog_generation').length,
      enhanced: items.filter(item => item.evidence_mode === 'isolated_catalog_generation').length,
