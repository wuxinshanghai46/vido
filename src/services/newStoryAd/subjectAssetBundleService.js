...
function personGenerationProfile(source = {}) {
  ...
  return {
    ...
    source_character_id: cleanText(source.source_character_id || source.character_id || source.id || '', 80),
    gender: cleanText(source.gender || 'unspecified', 24),
    age: cleanText(source.age || source.ageRange || source.appearance?.ageRange || 'match_brief', 40),
    age_range: cleanText(source.age_range || source.ageRange || source.age || '', 60),
    relationship: cleanText(source.relationship || '', 240),
    ethnicity: cleanText(source.ethnicity || source.ethnic_appearance || '', 160),
    voice_id: cleanText(source.voice_id || source.voice?.voice_id || '', 160),
    voice_tone: cleanText(source.voice_tone || source.voice?.direction || '', 300),
    performanceText: cleanText(source.performanceText || source.performance || '', 600),
    continuityText: cleanText(source.continuityText || source.continuity || '', 600),
...
function humanMemberSpecs(spec = {}, body = {}, count = 1) {
  ...
  return supplied.slice(0, count).map((source, index) => {
    ...
    return {
      ...
      source_character_id: cleanText(source.source_character_id || source.character_id || source.id || '', 80),
      gender: cleanText(source.gender || 'unspecified', 24),
      age_range: cleanText(source.age_range || source.ageRange || age || '', 60),
      relationship: cleanText(source.relationship || '', 240),
      ethnicity: cleanText(source.ethnicity || source.ethnic_appearance || '', 160),
      voice_id: cleanText(source.voice_id || source.voice?.voice_id || '', 160),
      voice_tone: cleanText(source.voice_tone || source.voice?.direction || '', 300),
      ...
      look_profiles: withLooks.look_profiles,
      look_name: cleanText(primary?.name || '', 160),
