...
function normalizeProfile(profile = {}, options = {}) {
  ...
  const continuity = clean(profile.identity_continuity || profile.identityContinuity || 'same_person', 40);
  ...
  const ageStates = (sourceStates.length ? sourceStates : [{
    ...
    const apparent = ageContract.normalize(state.apparent_age || state.apparentAge || state.age || baseAge.value);
    return {
      ...
      scene_ids: [...new Set((Array.isArray(state.scene_ids || state.sceneIds) ? (state.scene_ids || state.sceneIds) : [])
