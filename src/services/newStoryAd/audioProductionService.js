...
function savePlan(taskId, input = {}) {
  ...
  const nextPlan = {
    ...
    voice_volume: Math.max(0.6, Math.min(1.5, Number(input.voice_volume ?? state.plan.voice_volume ?? state.context.voice_volume ?? 1) || 1)),
    bgm_volume: Math.max(0, Math.min(1, Number(input.bgm_volume ?? state.plan.bgm_volume ?? state.context.bgm_volume ?? 0.16) || 0)),
