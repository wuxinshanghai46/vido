...
function eraCastContract(brief = '') {
  const value = text(brief, 3000);
  const reincarnation = /转生|转世|轮回|投胎|再世|来生|后世化身|前世的(?:转生|转世|后世)/.test(value);
  ...
  if (reincarnation) {
    ...
    return {
      ...
      distinct_roles: true,
      rule: '人物关系识别硬约束：转生、转世、轮回、投胎、来生或后世化身是新的独立人物身份，必须拥有独立正式姓名，禁止沿用前世姓名；只有原文明确本人穿越、共同穿越、长生者本人活到现代或同一身份时，才能保持原姓名。',
    };
  }
