...
function createService(deps = {}) {
  ...
  function preview(taskId = '', body = {}) {
    ...
    if (!activeCastCompatible) differences.push({ scope: 'authority', relation: 'positive_changed', field: 'cast_profiles', field_path: 'cast_profiles', reason_code: 'active_cast_changed', before: safeSummary(storage, activeCast), after: safeSummary(storage, currentCast), action: 'review_required', message: '当前人物内容与已确认依据存在差异' });
    ...
    if (!currentEligibility.eligible && !repairedContext) differences.push({ scope: 'lineage', relation: 'unproven', field: 'input_fingerprint', field_path: 'input_fingerprint', reason_code: 'lineage_unproven', before: safeSummary(storage, currentFingerprint), after: safeSummary(storage, activeFingerprint), action: 'review_required', message: '当前内容与已确认生成依据不一致' });
    const ready = Boolean(checkpoint && !unsafeBilling && currentEligibility.eligible);
    const rebase = Boolean(checkpoint && !unsafeBilling && !ready && repairedContext && repairedEligibility.eligible);
    ...
    const proofPayload = {
      ...
      active_fingerprint: activeFingerprint, repaired_fingerprint: repairedFingerprint,
    };
    return {
      ...
      model_call_count: 0, repaired_context: repairedContext,
    };
  }
  ...
  function apply(taskId = '', body = {}, expectedProofToken = '') {
    ...
    if (before.state !== 'safe_rebase_available' || !before.repaired_context) {
