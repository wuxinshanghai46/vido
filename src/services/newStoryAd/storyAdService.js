...
async function updatePersonPlan(taskId, options = {}) {
  ...
  const report = async (phase, message, extra = {}) => {
    if (typeof options.onProgress === 'function') await options.onProgress({
      phase, message, total: profiles.length, active_indexes: [...active].sort((a, b) => a - b).map(index => index + 1), ...extra,
    });
  };
  const scheduled = profiles.map((profile, index) => generationConcurrency.schedule(
    ...
    async () => {
      ...
      try {
        ...
        const forcedProfiles = profiles.map((item, itemIndex) => itemIndex === index ? forcedProfile : item);
        const response = await assistBrief({
          ...ctx,
          task_id: taskId,
          mode: 'person_spec',
          cast_profiles: forcedProfiles,
        ...
        const merged = { ...profile, ...generated,
          id: profile.id,
