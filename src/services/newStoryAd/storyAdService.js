...
const { buildSoundJourney } = require('./soundJourneyService'), soundDesignAssets = require('./soundDesignAssetService');
...
const { createTaskViewService } = require('./taskViewService'), assetPlanPublication = require('./assetPlanPublicationService'), blueprintActivePlanRebase = require('./blueprintActivePlanRebaseService'), releaseBundle = require('../storyAdReleaseBundleService');
...
function updateTaskRequest(taskId, body = {}, user = {}, options = {}) {
  ...
  storyboardCoverageLifecycle.assertConfirmationReady(storage, taskId, body);
...
function updateBlueprint(taskId, blueprint = {}, user = {}, options = {}) {
  ...
  if (changed) {
    const activePlanPreflight = blueprintActivePlanRebase.prepare(taskId, { motionOnly: preserveStoryboardForMotion });
    ...
    blueprintActivePlanRebase.carry(taskId, { contentRevision: nextRevision,
      motionOnly: preserveStoryboardForMotion, hadActivePlan: activePlanPreflight.had_active_plan });
...
async function generateStoryboardStage(taskId, options = {}) {
  ...
  storage.saveOutput(taskId, 'storyboard_meta', {
    ...
    blueprint_fingerprint: sourceFingerprint, story_flow_contract_fingerprint: storyFlowContract.contract_fingerprint,
  ...
  const saveCheckpoint = storyboardCoverageLifecycle.checkpointWriter({
    ...
    storyFlowContractFingerprint: storyFlowContract.contract_fingerprint,
...
async function buildKeyframeContractStage(taskId) {
  ...
  const storyFlowContract = storage.getOutput(taskId, 'story_flow_contract') || {}; shots = bindShotsToScenes(storyboardFlowConsistency.rebaseWhenPresent(shots, storyFlowContract, { boundary: 'keyframe_contract_build' }).shots, ctx.scene_assets); storyboardFlowConsistency.assertWhenPresent(shots, storyFlowContract, { boundary: 'keyframe_contract_build' });
  const blueprint = storage.getOutput(taskId, 'blueprint') || {};
...
function previewShotPrompts(taskId, options = {}) {
  ...
  const storyFlowContract = storage.getOutput(taskId, 'story_flow_contract') || {}; let boundShots = bindShotsToScenes(storyboardFlowConsistency.rebaseWhenPresent(shots, storyFlowContract, { boundary: 'prompt_preview' }).shots, ctx.scene_assets); storyboardFlowConsistency.assertWhenPresent(boundShots, storyFlowContract, { boundary: 'prompt_preview' });
...
async function generateKeyframesStage(taskId, options = {}) {
  ...
  const storyFlowContract = storage.getOutput(taskId, 'story_flow_contract') || {}; const boundShots = bindShotsToScenes(storyboardFlowConsistency.rebaseWhenPresent(shots, storyFlowContract, { boundary: 'keyframe_generation' }).shots, ctx.scene_assets); storyboardFlowConsistency.assertWhenPresent(boundShots, storyFlowContract, { boundary: 'keyframe_generation' });
...
async function generateTtsStage(taskId, options = {}) {
  nativeAudio.assertPostproduction(taskId, storage);
  const task = storage.getTask(taskId);
  if (!task) throw new Error('没有找到对应项目。');
...
function buildVideoPreflightPlan(taskId, options = {}) {
  ...
  const shots = nativeAudio.prepareShots(Array.isArray(storage.getOutput(taskId, 'storyboard_table')) ? storage.getOutput(taskId, 'storyboard_table') : []);
  ...
  videoSubmissionGate.addInputBlocker(plan, () => { assertVideoInputsReady({ ctx: contractCtx, shots, keyframes, contracts, sceneAcceptance: sceneVerificationOptions(taskId).acceptance }); if (shots.some(nativeAudio.wantsSound)) nativeAudioQa.candidate(); });
...
async function generateVideoStage(taskId, options = {}) { options = paidExecutionPolicy.canonicalize(options);
  ...
  const shots = nativeAudio.prepareShots(await ensureStoryboardForMedia(taskId));
  ...
  const generationShots = preflightPlan.reconciled_shots || shots;
  ...
  const ttsAudio = { tracks: [], mode: nativeAudio.MODE };
  ctx = nativeAudio.context(projectVideoOutputContext(ctx, options));
  storage.saveOutput(taskId, 'media_runtime_context', ctx);
  ...
  const qaDeferral = qaAvailability.createDeferral({ storage, videoAdapter, videoCostAuthorization, taskId, shots }); async function reviewVideoIndexes(reviewedIndexes = [], repairAttempt = 0, { stopOnFailure = false } = {}) {
    ...
    for (const index of reviewedIndexes) {
      ...
      videoAdapter.updateVideoShotStatus(taskId, index, {
        ...
        error: clips[index].error, error_code: clips[index].error_code, retryable: !qa.pass,
      }, shots.length);
      if (!qa.pass) {
...
function acceptVideoClipOverride(taskId, shotIndex, input = {}, user = {}) {
  ...
  nativeAudioQa.assertVerified(clip, shots[index]);
...
async function composeStage(taskId, options = {}) {
  const task = storage.getTask(taskId);
  if (!task) throw new Error('没有找到对应项目。');
  let ctx = nativeAudio.context(mediaRuntimeContext(taskId, storage.getOutput(taskId, 'context') || task.request || {}));
  const applyAudioEdits = options.apply_audio_edits === true;
  if (applyAudioEdits) nativeAudio.assertPostproduction(taskId, storage);
  const shots = nativeAudio.prepareShots(await ensureStoryboardForMedia(taskId));
  ...
  const composeKeyframes = approvedVideoFrames(taskId, shots, composeContracts), composeBlueprint = storage.getOutput(taskId, 'blueprint') || {}, composeStoryboardMeta = storage.getOutput(taskId, 'storyboard_meta') || {}, composeAudioTracks = [];
  ...
  shots.forEach((shot, index) => nativeAudioQa.assertVerified(clips[index], shot));
  ...
  const replacement = applyAudioEdits ? audioProduction.assertApproved(taskId) : null;
  const ttsAudio = replacement?.tts || { tracks: [] };
  const composeVoiceId = replacement?.voice_id || '', composeVoiceAssignments = replacement?.voice_assignments || {};
  const includeVoiceover = !!replacement?.include_voiceover;
  ...
  const bgmAsset = !applyAudioEdits ? null : hasBgmAssetOption
  ...
  const timelineEdits = storyAdTimeline.get(taskId), timelineTransitions = shots.map((shot, index) => ({ ...shot, ...(timelineEdits[index] || {}) }));
  const final_video = await composeService.concatVideos({
    ...
    brandOverlay,
    reviewFinalAudio: applyAudioEdits ? candidate => nativeAudioQa.reviewFinal({ taskId, candidate, shots }) : null,
    targetDurationSec: 0, soundTracks: applyAudioEdits ? soundDesignAssets.resolvedTracks(taskId) : [], timelineEdits, replaceSourceAudio: applyAudioEdits,
  });
  const finalVideoWithLineage = {
    ...final_video,
    audio_mode: applyAudioEdits ? 'postproduction_replacement' : nativeAudio.MODE,
    ...
    audio_license_ledger_fingerprint: storage.canonicalFingerprint(storage.getOutput(taskId, soundDesignAssets.LEDGER_KIND) || []), attribution_manifest: soundDesignAssets.attributionManifest(taskId),
  };
