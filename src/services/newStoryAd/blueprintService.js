...
function looksLikeChinesePersonName(value = '') {
  ...
  if (['通过', '随后', '然后', '镜头', '画面', '指尖', '手掌', '墙面', '光线', '人物', '背景', '表情', '侧光', '顶光', '点光', '样片', '纹理'].includes(name)) return false;
...
function normalizeBlueprint(blueprint, ctx) {
  ...
  const normalizedBeats = beats.map((beat, idx) => {
    ...
    return {
      ...
      camera_movement_notes: clean(beat.camera_movement_notes || '', 320),
      shot_size: clean(beat.shot_size || beat.shot_type || beat.camera || '', 80),
      sound_design: clean(beat.sound_design || beat.sound || beat.audio || '', 160),
      sound_contract_version: 1,
      sound_mode: clean(beat.sound_mode || '', 30) || ((beat.ambient_sound || beat.sfx || beat.music_cue || beat.sound_design) ? 'designed' : 'unassigned'),
      ambient_sound: clean(beat.ambient_sound || '', 240),
      sfx: cleanList(beat.sfx || beat.sound_effects, 12, 160),
      music_cue: clean(beat.music_cue || '', 240),
      audio_bridge: clean(beat.audio_bridge || '', 240),
      explicit_silence_reason: clean(beat.explicit_silence_reason || '', 240),
      voiceover_timing: clean(beat.voiceover_timing || '', 240),
...
Return JSON in this shape:
{
  ...
  "beats": [{
    ...
    "camera_movement_notes": "optional camera movement timing and execution detail",
    "shot_size": "wide/full/medium/medium_close/close_up/extreme_close_up/insert",
    "sound_design": "environment sound, effect and music cue",
    "sound_mode": "designed/ambient_only/silent",
    "ambient_sound": "specific environment bed; required unless silent",
    "sfx": ["specific synchronized action sound effects"],
    "music_cue": "music mood and entry/exit cue; empty only when intentionally unnecessary",
    "audio_bridge": "audio continuity into the next shot",
    "explicit_silence_reason": "required reason only when sound_mode is silent",
    "voiceover_timing": "spoken line timing inside the shot",
    "transition": "continuity or transition into the next beat",
    "prompt_notes": "production prompt notes before asset binding",
    "keyframe_prompt_override": "optional user-editable keyframe prompt override",
    "video_prompt_override": "optional user-editable video prompt override",
    "negative_prompt_override": "optional user-editable negative prompt override",
