...
function outputSchema() {
  ...
  "scene_plan": {
    ...
    "spaces": [{
      ...
      "scene_spec": {
        "layoutText": "仅属于这个地点的空间布局、主体位置、前景/背景关系和可复用空间身份，80-180 字",
        "materialLightText": "仅属于这个地点的材质、色彩、光线方向和真实拍摄质感，80-180 字",
        "interactionText": "仅属于这个地点的人物/商品动作区、通行路线和可用机位，60-140 字",
        "negativeText": "这个地点不得出现的其它空间、结构、材质、人物、文字水印和无关元素，分号分隔",
        "surfaceTopology": {
...
function enforceAssistedScenePlan(raw = {}, currentPlan = {}, currentSpec = {}, context = {}) {
...
function buildResponse({ parsed = {}, context = {}, currentPlan = {}, mode = 'scene_spec', modelResult = {} } = {}) {
  ...
  return {
    scene_plan: plan,
    scene_config: plan,
    scene_spec: plan.spaces[0]?.scene_spec || null,
    mode,
    model_meta: {
