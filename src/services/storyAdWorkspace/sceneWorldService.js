...
function productionManifest(bundle = {}, worlds = [], options = {}) {
  ...
  const animalMatrix = list(bundle.assets?.animals).map((animal, index) => {
    ...
    return {
      ...
      cells: worlds.map(world => {
        ...
        return {
          world_id: world.id,
          presence: clean(explicit?.presence, 30) || (matched.length ? 'confirmed' : (worlds.length === 1 ? 'suggested' : 'unassigned')),
          shot_count: matched.length,
          role: clean(explicit?.role || matched.map(shot => shot.action).filter(Boolean).join('；'), 260),
