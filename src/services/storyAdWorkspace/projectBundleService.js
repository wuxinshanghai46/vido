...
function buildProjectBundle(taskId, { sections = '', user = {} } = {}) {
  ...
  const project = {
    ...
    video_submission_failure: failureProjection.submissionFailure(storage.getStage(taskId, 'video_submission'), canViewErrors, { activePlanEligible: outputs.asset_plan_eligibility?.eligible === true }),
  ...
  const countableAssets = countAssetsForPartialWorkspace ? {
    ...
    scenes: include('assets') ? projectedScenesForCounts : (persistedScenesForCounts.length ? persistedScenesForCounts : projectedScenesForCounts),
  } : null;
