...
- 眼球湿润高光、眼睑厚度、唇纹、发际碎发和面部软组织运动彼此一致；
- 避免千人一面的高鼻梁、窄下颌和默认网红笑，保留角色可识别的骨相比例与细小不对称；
- 表情由剧情动作触发，视线、眼睑、眉部、脸颊、嘴角与下颌肌肉必须协同，避免“嘴笑眼不笑”和情绪戏中的证件照表情；
- 人物主光、补光、色温、轮廓光与接触阴影必须服从现场环境，避免人物像抠图贴在背景上；
- 更换景别、服装或背景后仍保持同一演员的脸部比例、年龄感和识别特征。
...
禁止把网格、遮挡、马赛克、变形或其他处理当作“过审核”方案；审核失败应停止生成，核验授权或改用原创合成人物。`,
    ...
    source: 'VIDO synthesis from public face-realism observations and the WeChat article《为什么你刷到的AI视频，翻来覆去都是同一张脸...》, reviewed 2026-07-24',
    source_url: 'https://mp.weixin.qq.com/s/SV0T0W6hSpJYxweuhSoo0A',
    lang: 'zh',
    enabled: true,
  },
  {
...
不得通过拆词、错别字、遮挡、变形或其他方式规避供应商审核。`,
  ...
  {
    id: 'kb_gpt_image2_domestic_compliance_preflight',
    collection: 'engineering',
    subcategory: 'GPT Image 2 Compliance',
    title: '国内 GPT Image 2 生成前合规检查与低返工协议',
    summary: '把人物授权、成人年龄、版权元素、参考图数量和审核型错误处理前置到提示词与任务契约，减少无效抽卡和重复付费。',
    content: `本规则用于合规前置和减少无效生成，不用于规避供应商审核。

生成前检查：
1. 涉及人物时明确为成年人，并写清具体成人年龄、自然非暗示姿势和正常商业场景；未成年人、色情、血腥暴力、仇恨、政治敏感和隐私提取内容直接停止。
2. 人脸只允许原创合成人物、本人明确授权或具备可验证素材许可的肖像。名人、公众人物和来源不明人脸不得进入一致性生成。
3. 未核验权利时，不使用受保护角色、品牌、Logo、作品名、在世创作者姓名或可识别的标志性风格。把风格诉求改写为可观察的灯光、色彩、镜头、材质与构图参数。
4. 单张参考图必须小于 25MB；虽然接口可接受更多素材，默认最多使用 6 张最相关参考图，删除重复、矛盾、低清或无授权素材。
5. 人物素材使用清晰、单人、正面无遮挡的静态图。当前剧情广告沿用正面全身参考图进入人物素材接口，不额外生成或上传大头照；虚拟人物进入 AIGC 素材组，真人仅在授权流程完成后进入真人组。默认只传图片，不传视频。

审核失败处理：
- 明确审核拒绝、隐私、肖像、版权或敏感内容错误均不可自动重试。
- 模糊 5xx 也按审核风险暂停，不进行高频或重复付费调用。
- 保留已经成功且契约仍兼容的视图；只修正失败视图并生成新指纹。
- 修正顺序是删除无关素材、缩短并明确提示词、核验成人与授权状态、移除未授权 IP/品牌/名人元素，再由用户确认是否重新提交。
- 禁止拆词、错别字、遮挡、变形、诱导描述或任何其它审核规避手段。`,
    tags: ['GPT Image 2', '国内审核', '人物授权', '成人年龄', '停止重试', '低返工'],
    keywords: ['25MB', '最多6张参考图', 'AIGC人物素材', '正面全身参考图', 'moderation preflight', 'Image 2 compliance'],
    prompt_snippets: [
      '人物合规前置：明确成年人和具体年龄，使用自然非暗示姿势；仅使用原创合成人物或可验证授权肖像。',
      '风格合规改写：删除未授权名人、角色、品牌、Logo、作品名和在世创作者姓名，改写为可观察的光线、色彩、镜头、材质与构图参数。',
      '参考图默认不超过 6 张且单张小于 25MB；人物沿用正面全身静态参考图，虚拟人物进入 AIGC 素材组，不额外生成或上传大头照与视频。',
    ],
    applies_to: ['director', 'art_director', 'prompt_engineer', 'project_assistant', 'test_engineer'],
    source: '供应商资料《国内GPT Image 2内容审核规则及规避指南》合规重写，2026-07-24 审阅',
    lang: 'zh',
    enabled: true,
  },
];
