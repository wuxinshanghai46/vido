export async function submitUnifiedProductionAssets({ button, bundle, request, confirmDialog, store, setButtonBusy, toast }) {
  try {
    ...
    const plan = await request(`/api/new-story-ad/tasks/${encodeURIComponent(bundle.project.id)}/production-assets/plan`, {
      method: 'POST', body: { generate_panoramas: true }, timeoutMs: 120000,
    });
    ...
    const authorizedLimit = Number(plan.maximum_confirmable_cost_rmb || plan.confirmed_cost_limit_rmb || 0);
    const accepted = await confirmDialog(`本次会生成完整人物、穿搭配饰、随身物、动作表情、场景母图、360°全景、机位和逐镜执行合同。服务端保守预估最多 ${calls} 次模型调用，其中付费图片最多 ${paidCalls} 次、视觉费用上限约 ${visualCost} 元；超过本次 ${authorizedLimit.toFixed(2)} 元授权上限会在图片模型调用前停止。`, {
    ...
    await store.runStage('production-assets', { generate_panoramas: true, cost_confirmation: true,
      confirmed_cost_limit_rmb: authorizedLimit, plan_fingerprint: plan.plan_fingerprint });
