export async function submitUnifiedProductionAssets({ button, bundle, request, confirmDialog, store, setButtonBusy, toast }) {
  try {
    ...
    const accepted = await confirmDialog(`本次会生成完整人物、穿搭配饰、随身物、动作表情、场景母图、五机位和逐镜执行合同。360°全景为后续可选增强，不会阻塞本次普通视频制作。服务端保守预估最多 ${calls} 次模型调用，其中付费图片最多 ${paidCalls} 次、视觉费用上限约 ${visualCost} 元；超过本次 ${authorizedLimit.toFixed(2)} 元授权上限会在图片模型调用前停止。`, {
