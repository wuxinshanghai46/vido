export async function submitUnifiedProductionAssets({ button, bundle, request, confirmDialog, store, setButtonBusy, toast, imageModel = '' }) {
  try {
    ...
    const plan = await request(`/api/new-story-ad/tasks/${encodeURIComponent(bundle.project.id)}/production-assets/plan`, {
      method: 'POST', body: { spatial_mode: 'multi_view', generate_panoramas: false, image_model: imageModel }, timeoutMs: 120000,
    });
    await store.runStage('production-assets', { spatial_mode: 'multi_view', generate_panoramas: false, cost_confirmation: true,
      image_model: imageModel, plan_fingerprint: plan.plan_fingerprint });
