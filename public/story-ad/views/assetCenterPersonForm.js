...
export function personEditForm(item = {}) {
  ...
  const generationType = String(runtime.generation_type || settings.generation_type || 'three_view');
  ...
  const supportedResolutions = new Set(runtime.supported_resolutions || ['1K', '2K']);
  return `<section class="person-edit-panel person-prompt-workbench" data-person-prompt-workbench><header class="person-prompt-head"><div><small>人物生成提示词</small><h3>${escapeHtml(profile.displayName || item.name || '未命名人物')}</h3><p>剧情生成时已写好完整提示词；点击人物即可查看、直接修改。</p></div><div><span>模型生成</span><span>${escapeHtml(item.status || 'draft')}</span></div></header><form id="personEditForm" data-person-edit>
    ...
    <footer class="person-prompt-toolbar" aria-label="人物图片生成设置"><div class="person-prompt-settings">
      ...
      <label class="person-setting-select" title="广告默认 3 视图，剧情默认全局整图；数量固定为 1"><span class="sr-only">生成类型</span><select name="generation_type">
        ${option('three_view', '3视图', generationType)}${option('four_view', '4视图', generationType)}${option('global_dossier', '全局整图', generationType)}
      </select></label>
      <label class="person-setting-select"><span class="sr-only">画质</span><select name="quality">
        ${option('low', '低画质', quality)}${option('standard', '标准画质', quality)}${option('high', '高画质', quality)}
      </select></label>
      <label class="person-setting-select"><span class="sr-only">清晰度</span><select name="resolution">
        ${option('1K', '1K', resolution, !supportedResolutions.has('1K'))}${option('2K', '2K', resolution, !supportedResolutions.has('2K'))}${option('4K', '4K（当前模型不支持）', resolution, !supportedResolutions.has('4K'))}
      </select></label>
