...
import { bindMediaLightbox, emptyState, escapeHtml, setButtonBusy, toast } from '../components/ui.js?v=20260815-reference-transition-v59';
import { confirmDialog } from '../components/dialog.js?v=20260815-reference-transition-v59';
import { openActorLibrary, openRealPersonFlow } from './assetCenterPersonSources.js?v=20260815-reference-transition-v59';
import { authorizeBillingReviews, confirmBillingAwareAction } from './assetCenterBillingRetry.js?v=20260815-reference-transition-v59';
import { collectPersonLookValues, renderPersonLookTiles } from './assetCenterPersonLooks.js?v=20260815-reference-transition-v59';
import { legacyDossierBoard, mediaSection } from './assetCenterDossierSections.js?v=20260815-reference-transition-v59';
import { assetCardMedia } from './sceneDossierCard.js?v=20260815-reference-transition-v59';
import { assertSavedPerson, personAgeDisplay, personAssetState, personLookSummary } from './assetCenterPersonState.js?v=20260815-reference-transition-v59';
import { bindPersonEvolutionForm, collectPersonEvolutionValues, renderPersonEvolutionSummary } from './assetCenterPersonEvolution.js?v=20260815-reference-transition-v59';
import { createPersonPlanRequestGuard, personPlanBlockedView } from './assetCenterPlanReleaseStatus.js?v=20260815-reference-transition-v59';
const GROUPS = [
  ['people', '人物'],
  ['animals', '动物'],
  ['products', '商品 / 展示主体'],
...
let planningDetailsPromise; let personFormPromise; async function openDrawer(item, group, handlers = {}) {
  planningDetailsPromise ||= import('./assetCenterPlanningDetails.js?v=20260815-reference-transition-v59');
  personFormPromise ||= import('./assetCenterPersonForm.js?v=20260815-reference-transition-v59');
...
export async function mount(host, context) {
  ...
  const runAssist = async (kind, ...args) => (await (assistModulePromise ||= import('./assetCenterAssist.js?v=20260815-reference-transition-v59'))).createAssetAssistHandlers(bundle)[kind](...args);
  ...
  host.querySelector('[data-update-person-plan]')?.addEventListener('click', async event => {
    ...
    await personPlanRequestGuard.run(async (requestKey) => {
      const { submitPersonPlanUpdate } = await import('./assetCenterPlanMigrationAction.js?v=20260815-reference-transition-v59');
