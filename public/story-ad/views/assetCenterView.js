...
import { openActorLibrary, openRealPersonFlow } from './assetCenterPersonSources.js?v=20260822-reference-extended-analysis-v141';
import { authorizeBillingReviews, bindSubjectBillingRecovery, confirmBillingAwareAction, ensureSubjectRecoveryReady, recoveryRequestKey } from './assetCenterBillingRetry.js?v=20260822-reference-extended-analysis-v141';
import { collectPersonLookValues, renderPersonLookTiles } from './assetCenterPersonLooks.js?v=20260822-reference-extended-analysis-v141';
import { legacyDossierBoard, mediaSection } from './assetCenterDossierSections.js?v=20260822-reference-extended-analysis-v141';
import { assetCardMedia } from './sceneDossierCard.js?v=20260822-reference-extended-analysis-v141';
import { assertSavedPerson, personAgeDisplay, personAssetState, personLookSummary } from './assetCenterPersonState.js?v=20260822-reference-extended-analysis-v141';
import { bindPersonEvolutionForm, collectPersonEvolutionValues, renderPersonEvolutionSummary } from './assetCenterPersonEvolution.js?v=20260822-reference-extended-analysis-v141';
import { createKeyedRequestGuard, createPersonPlanRequestGuard } from './assetCenterPlanReleaseStatus.js?v=20260822-reference-extended-analysis-v141';
...
const loadCheckpointRecovery = globalThis.__loadAssetCheckpointRecovery
  || (() => import('./assetCheckpointRecovery.js?v=20260822-reference-extended-analysis-v141'));
const loadAssetCenterStage = globalThis.__loadAssetCenterStage || (() => import('./assetCenterStageView.js?v=20260822-reference-extended-analysis-v141'));
...
let planningDetailsPromise; let personFormPromise; async function openDrawer(item, group, handlers = {}) {
  planningDetailsPromise ||= import('./assetCenterPlanningDetails.js?v=20260822-reference-extended-analysis-v141');
  personFormPromise ||= import('./assetCenterPersonForm.js?v=20260822-reference-extended-analysis-v141');
...
export async function mount(host, context) {
  ...
  const runAssist = async (kind, ...args) => (await (assistModulePromise ||= import('./assetCenterAssist.js?v=20260822-reference-extended-analysis-v141'))).createAssetAssistHandlers(bundle)[kind](...args);
  ...
  host.querySelector('[data-update-person-plan]')?.addEventListener('click', async event => {
    ...
    await personPlanRequestGuard.run(async (requestKey) => {
      const { submitPersonPlanUpdate } = await import('./assetCenterPlanMigrationAction.js?v=20260822-reference-extended-analysis-v141');
