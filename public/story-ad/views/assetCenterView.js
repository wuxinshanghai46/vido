...
import { emptyState, escapeHtml, setButtonBusy, toast } from '../components/ui.js?v=20260826-production-v230p';
...
import { openActorLibrary, openRealPersonFlow } from './assetCenterPersonSources.js?v=20260826-production-v230p';
import { ensureSubjectRecoveryReady, recoveryRequestKey } from './assetCenterBillingRetry.js?v=20260826-production-v230p';
import { renderPersonLookTiles } from './assetCenterPersonLooks.js?v=20260826-production-v230p';
import { mediaSection } from './assetCenterDossierSections.js?v=20260826-production-v230p';
import { assetCardMedia } from './sceneDossierCard.js?v=20260826-production-v230p';
import { assertSavedPerson, personAgeDisplay, personAssetState, personLookSummary } from './assetCenterPersonState.js?v=20260826-production-v230p';
import { renderPersonEvolutionSummary } from './assetCenterPersonEvolution.js?v=20260826-production-v230p';
import { createKeyedRequestGuard } from './assetCenterRequestGuard.js?v=20260826-production-v230p';
import { checkpointRecoverySummary } from './assetCheckpointRecovery.js?v=20260826-production-v230p';
if (typeof document !== 'undefined' && !document.getElementById('person-dossier-style')) {
  const style = document.createElement('link');
  style.id = 'person-dossier-style';
  style.rel = 'stylesheet';
  style.href = '/story-ad/person-dossier.css?v=20260826-production-v230p';
  document.head.append(style);
}
const GROUPS = [['people', '人物'], ['animals', '动物'], ['products', '商品 / 展示主体'], ['logos', 'LOGO']];
const GENERATABLE = new Set(['people', 'animals']);
const loadAssetCenterStage = globalThis.__loadAssetCenterStage || (() => import('./assetCenterStageView.js?v=20260826-production-v230p'));
...
let planningDetailsPromise; let personFormPromise; async function openDrawer(item, group, handlers = {}) {
  planningDetailsPromise ||= import('./assetCenterPlanningDetails.js?v=20260826-production-v230p');
  personFormPromise ||= import('./assetCenterPersonForm.js?v=20260826-production-v230p');
...
export async function mount(host, context) {
  ...
  const runAssist = async (kind, ...args) => (await (assistModulePromise ||= import('./assetCenterAssist.js?v=20260826-production-v230p'))).createAssetAssistHandlers(bundle)[kind](...args);
