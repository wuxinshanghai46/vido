import { request } from '../api.js?v=20260822-provider-v164';
import { emptyState, escapeHtml, setButtonBusy, toast } from '../components/ui.js?v=20260822-provider-v164';
import { bindMediaLightbox } from './mediaLightbox.js?v=20260822-provider-v164';
import { confirmDialog } from '../components/dialog.js?v=20260822-provider-v164';
import { openActorLibrary, openRealPersonFlow } from './assetCenterPersonSources.js?v=20260822-provider-v164';
import { authorizeBillingReviews, bindSubjectBillingRecovery, confirmBillingAwareAction, ensureSubjectRecoveryReady, recoveryRequestKey } from './assetCenterBillingRetry.js?v=20260822-provider-v164';
import { collectPersonLookValues, renderPersonLookTiles } from './assetCenterPersonLooks.js?v=20260822-provider-v164';
import { legacyDossierBoard, mediaSection } from './assetCenterDossierSections.js?v=20260822-provider-v164';
import { assetCardMedia } from './sceneDossierCard.js?v=20260822-provider-v164';
import { assertSavedPerson, personAgeDisplay, personAssetState, personLookSummary } from './assetCenterPersonState.js?v=20260822-provider-v164';
import { bindPersonEvolutionForm, collectPersonEvolutionValues, renderPersonEvolutionSummary } from './assetCenterPersonEvolution.js?v=20260822-provider-v164';
import { createKeyedRequestGuard, createPersonPlanRequestGuard } from './assetCenterPlanReleaseStatus.js?v=20260822-provider-v164';
...
const loadCheckpointRecovery = globalThis.__loadAssetCheckpointRecovery
  || (() => import('./assetCheckpointRecovery.js?v=20260822-provider-v164'));
const loadAssetCenterStage = globalThis.__loadAssetCenterStage || (() => import('./assetCenterStageView.js?v=20260822-provider-v164'));
...
let planningDetailsPromise; let personFormPromise; async function openDrawer(item, group, handlers = {}) {
  planningDetailsPromise ||= import('./assetCenterPlanningDetails.js?v=20260822-provider-v164');
  personFormPromise ||= import('./assetCenterPersonForm.js?v=20260822-provider-v164');
...
export async function mount(host, context) {
  ...
  const runAssist = async (kind, ...args) => (await (assistModulePromise ||= import('./assetCenterAssist.js?v=20260822-provider-v164'))).createAssetAssistHandlers(bundle)[kind](...args);
  ...
  host.querySelector('[data-update-person-plan]')?.addEventListener('click', async event => {
    ...
    await personPlanRequestGuard.run(async (requestKey) => {
      const { submitPersonPlanUpdate } = await import('./assetCenterPlanMigrationAction.js?v=20260822-provider-v164');
