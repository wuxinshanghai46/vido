...
import { emptyState, escapeHtml, setButtonBusy, toast } from '../components/ui.js?v=20260904-production-v432';
import { bindMediaLightbox } from './mediaLightbox.js?v=20260904-production-v432';
import { confirmDialog } from '../components/dialog.js?v=20260904-production-v432';
import { openActorLibrary, openRealPersonFlow } from './assetCenterPersonSources.js?v=20260904-production-v432';
import { ensureSubjectRecoveryReady, recoveryRequestKey } from './assetCenterBillingRetry.js?v=20260904-production-v432';
import { renderPersonLookTiles } from './assetCenterPersonLooks.js?v=20260904-production-v432';
import { mediaSection } from './assetCenterDossierSections.js?v=20260904-production-v432';
import { assetCardMedia } from './sceneDossierCard.js?v=20260904-production-v432';
import { assertSavedPerson, personAgeDisplay, personAssetState, personLookSummary } from './assetCenterPersonState.js?v=20260904-production-v432';
import { renderPersonEvolutionSummary } from './assetCenterPersonEvolution.js?v=20260904-production-v432';
import { createKeyedRequestGuard } from './assetCenterRequestGuard.js?v=20260904-production-v432';
import { checkpointRecoverySummary } from './assetCheckpointRecovery.js?v=20260904-production-v432';
import { bindGenerationModelPicker, loadGenerationModelPicker } from './generationModelPicker.js?v=20260904-production-v432';
if (typeof document !== 'undefined' && !document.getElementById('person-dossier-style')) {
  const style = document.createElement('link');
  style.id = 'person-dossier-style';
  style.rel = 'stylesheet';
  style.href = '/story-ad/person-dossier.css?v=20260904-production-v432';
  document.head.append(style);
}
const GROUPS = [['people', '人物'], ['animals', '动物'], ['products', '商品 / 展示主体'], ['logos', 'LOGO']];
const GENERATABLE = new Set(['people', 'animals']);
const loadAssetCenterStage = globalThis.__loadAssetCenterStage || (() => import('./assetCenterStageView.js?v=20260904-production-v432'));
...
let planningDetailsPromise; let personFormPromise; async function openDrawer(item, group, handlers = {}) {
  planningDetailsPromise ||= import('./assetCenterPlanningDetails.js?v=20260904-production-v432');
  personFormPromise ||= import('./assetCenterPersonForm.js?v=20260904-production-v432');
...
export async function mount(host, context) {
  ...
  const runAssist = async (kind, ...args) => (await (assistModulePromise ||= import('./assetCenterAssist.js?v=20260904-production-v432'))).createAssetAssistHandlers(bundle)[kind](...args);
