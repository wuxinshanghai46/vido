...
import { applyBeat, beatEditor, collectBeat, collectBlueprint, productionIssues, syncFloatingEditor } from './plotBeatEditor.js?v=20260904-production-v449';
...
export async function mount(host, context) {
  ...
  const characterAutosave = characters.length
    ? (await import('./plotCharacterAutosave.js?v=20260904-production-v449'))
  ...
  const openEditor = async (button, row, group) => {
    if (group === 'prompt_notes') {
      promptModule ||= await import('./plotPromptPreview.js?v=20260904-production-v449');
      await promptModule.openPromptPreview({ pop, row, host, projectId: bundle.project.id, place: () => place(button), closeAll });
      return;
    }
    cellEditorModule ||= await import('./plotBeatCellPopover.js?v=20260904-production-v449');
