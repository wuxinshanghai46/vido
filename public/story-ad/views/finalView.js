import { emptyState, escapeHtml, mediaPreview, setButtonBusy, toast } from '../components/ui.js?v=20260903-production-v418';
import { bindMoreMedia, moreMediaButton } from './finalMediaPagination.js?v=20260903-production-v418';
...
export async function mount(host, context) {
  ...
  host.innerHTML = `
    ...
    <details class="card generation-section generation-details"><summary class="card-head"><div><h2>已确认分镜 / 视频首帧</h2><p>${approvedFrames.length}/${shots.length} · 直接进入图生视频，不重复生成图片</p></div><span class="details-chevron" aria-hidden="true">⌄</span></summary><div class="card-body">${approvedFrames.length ? `<div class="generation-grid">${approvedFrames.map((item, index) => mediaCard(item, index, '首帧')).join('')}</div>` : emptyState({ title: '分镜尚未完整', body: '请返回分镜页逐镜生成或重绘，然后确认镜头设计。', action: '返回人物场景分镜生成首帧', actionId: 'back-storyboard' })}</div></details>
    <section class="card generation-section"><div class="card-head"><div><h2>分镜视频</h2><p>已完成 ${clips.length}/${clipTotal || shots.length}</p></div></div><div class="card-body">${clips.length ? `<div class="generation-grid">${clips.map((item, index) => mediaCard(item, index, '视频')).join('')}</div>${moreMediaButton(mediaCatalog.clips, 'clips', '继续加载视频片段')}` : emptyState({ title: '还没有分镜视频', body: framesReady ? '声音已确认。选择视频模型并完成费用预检后，才能提交真实视频生成。' : `还缺少 ${Math.max(0, shots.length - approvedFrames.length)} 张已确认首帧，请先返回人物场景分镜生成并确认。`, action: framesReady ? '' : '返回人物场景分镜生成首帧', actionId: framesReady ? '' : 'back-storyboard' })}</div></section>
