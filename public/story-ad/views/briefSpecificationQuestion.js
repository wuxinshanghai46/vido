import { BRIEF_DURATION_OPTIONS, durationLabel } from './briefDurationOptions.js?v=20260829-production-v281b';
...
export function mountSpecificationQuestion(conversation, options = {}) {
  ...
  find('[data-spec-choice="apply"]')?.addEventListener('click', () => {
    ...
    options.onAdjust?.({
      ...
      resolution: String(find('[data-spec-resolution]')?.value || options.resolution || '1080p'),
    });
  });
  conversation.scrollTop = conversation.scrollHeight;
  return article;
