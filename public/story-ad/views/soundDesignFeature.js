...
import { bgmCandidateMarkup, bindLiveAudioPreview, previewSeconds, soundPreviewMarkup } from '../controllers/liveAudioPreviewController.js?v=20260904-production-v462';
import { recommendedVoice, speechShotCount, voiceSampleText } from './soundDesignVoiceCatalog.js?v=20260904-production-v462';
...
export function bindSoundDesign(host, { bundle, store, soundDesign = {}, refreshShell, navigate }) {
  ...
  host.querySelectorAll('[data-sound-shot]').forEach(row => {
    ...
    input?.addEventListener('change', async event => {
      ...
      try { setButtonBusy(button, true, '上传中…'); const uploaded = await store.upload(file, row.dataset.soundTrack === 'bgm' ? 'background_music' : 'sound_effect'); await request(`/api/story-ad/projects/${encodeURIComponent(bundle.project.id)}/sound-assets`, { method: 'POST', body: { asset: uploaded.asset || uploaded.data, shot_index: Number(row.dataset.soundShot), track_type: row.dataset.soundTrack || row.querySelector('[data-sound-track-type]')?.value || 'sfx' } }); toast(row.dataset.soundTrack === 'bgm' ? '已使用你上传的背景音乐。' : '音频已绑定到当前分镜。', 'success'); await refreshShell(); } catch (error) { toast(error.message, 'danger'); } finally { setButtonBusy(button, false); }
    });
  });
