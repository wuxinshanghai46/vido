import { escapeHtml, mediaPreview } from '../components/ui.js?v=20260824-production-v201ap';
import { bindMediaLightbox } from './mediaLightbox.js?v=20260824-production-v201ap';
import { personDossierShowcase } from './personDossierShowcase.js?v=20260824-production-v201ap';
import { bindSceneDossierCard, renderSceneDossierCard } from './sceneDossierCard.js?v=20260824-production-v201ap';
import { bindPersonLookForm } from './assetCenterPersonLooks.js?v=20260824-production-v201ap';
import { bindPersonEvolutionForm } from './assetCenterPersonEvolution.js?v=20260824-production-v201ap';
...
export function openAssetDrawer(item, group, handlers = {}, renderers = {}) {
  ...
  drawer.innerHTML = `<header class="drawer-head"><div><small>${escapeHtml(groupLabel)}</small><h2>${escapeHtml(item.name)}</h2></div><button class="icon-btn" type="button" data-close-drawer>×</button></header><div class="drawer-content">
    ...
    ${group === 'people' && !readOnly ? '<footer class="drawer-actions person-drawer-actions"><span>修改并保存后，请回到资产中心点击“生成全部制作资产”；系统只补受影响的人物、穿搭、随身物和下游绑定。</span><div><button class="btn primary" type="submit" form="personEditForm">保存人物文字设定</button></div></footer>' : ''}
    ${group === 'scenes' && !readOnly ? '<footer class="drawer-actions"><span>修改并保存后，请回到资产中心点击“生成全部制作资产”；旧场景不会继续影响新图谱。</span></footer>' : ''}
    ${group === 'products' ? `<footer class="drawer-actions product-reference-actions"><span>上传或更换主体图片后，由“生成全部制作资产”统一建立商品、场景和分镜绑定。</span><div>${readOnly ? '' : `<button class="btn" type="button" data-drawer-upload-product>${item.image_url ? '更换主体图片' : '上传主体图片'}</button>`}</div></footer>` : ''}
