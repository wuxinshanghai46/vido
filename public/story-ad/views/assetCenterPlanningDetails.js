import { escapeHtml, mediaPreview } from '../components/ui.js?v=20260904-production-v442';
import { bindMediaLightbox } from './mediaLightbox.js?v=20260904-production-v442';
import { personDossierShowcase } from './personDossierShowcase.js?v=20260904-production-v442';
import { bindSceneDossierCard, renderSceneDossierCard } from './sceneDossierCard.js?v=20260904-production-v442';
import { publicSceneQaReason } from './sceneQaPublicState.js?v=20260904-production-v442';
import { productDetails } from './assetCenterMaterialReference.js?v=20260904-production-v442';
...
function cameraPlanMap(item = {}, cameras = []) {
  ...
  return `<section class="scene-camera-map-section"><div class="drawer-section-head"><h3>机位调度与观看方向</h3><span>${plotted.length}/${cameras.length} 个机位已定位</span></div><p class="drawer-section-note">编号表示拍摄站位；紫色虚线表示整组镜头的机位调度顺序，不等同于单个镜头的实际运镜轨迹。每一段怎么拍请展开下方“动态拍摄细则”。</p><div class="scene-camera-map">
    ${mediaPreview({ image_url: item.layout.image_url }, { label: `${item.name}整体机位规划图`, width: 1800, zoomWidth: 1600, symbol: '机位规划', zoomable: true, zoomGroup: `scene-director-${item.id || 'current'}` })}
...
export function sceneDetails(item = {}) {
  ...
  return `<div class="scene-detail-stack">
    ...
    ${item.layout?.image_url ? `<details class="scene-layout raw-view-details"><summary>查看无标记的空间母版 / 布局原图</summary>${mediaPreview({ image_url: item.layout.image_url }, { label: `${item.name}空间布局原图`, width: 1200, zoomWidth: 1600, symbol: '空间布局', zoomable: true, zoomGroup: `scene-layout-original-${item.id || 'current'}` })}</details>` : ''}
...
export function openAssetDrawer(item, group, handlers = {}, renderers = {}) {
  ...
  const hasPersonMedia = group === 'people' && Boolean(
  ...
  drawer.innerHTML = `<header class="drawer-head"><div><small>${escapeHtml(groupLabel)}</small><h2>${escapeHtml(item.name)}</h2></div><button class="icon-btn" type="button" data-close-drawer>×</button></header><div class="drawer-content">
    ${personTabs}
    ${group === 'people' ? '' : (!views.length ? mediaPreview(item, { label: item.name, width: 1200, symbol: groupLabel, zoomable: true, zoomGroup: `asset-${item.id}` }) : '')}${group === 'people' ? '' : checkpointDetails(item)}
    ${group === 'scenes' ? sceneDetails(item) : ''}
    ${views.length ? (group === 'scenes' ? `<details class="raw-view-details"><summary>查看场景原始图集（${views.length} 张）</summary>${mediaSection('场景视角图集', views)}</details>` : (group === 'people' ? '' : mediaSection('完整视图', views, group === 'animals' ? 'is-portrait-grid' : ''))) : ''}
  ...
  if (editablePerson && drawer.querySelector('[name="generation_prompt"]')) import('./personPromptAutosave.js?v=20260904-production-v442').then(module => {
