import { escapeHtml, mediaPreview } from '../components/ui.js?v=20260822-reference-first-compact-dialogue-v135';
import { bindMediaLightbox } from './mediaLightbox.js?v=20260822-reference-first-compact-dialogue-v135';
import { personDossierShowcase } from './personDossierShowcase.js?v=20260822-reference-first-compact-dialogue-v135';
import { bindSceneDossierCard, renderSceneDossierCard } from './sceneDossierCard.js?v=20260822-reference-first-compact-dialogue-v135';
import { bindPersonLookForm } from './assetCenterPersonLooks.js?v=20260822-reference-first-compact-dialogue-v135';
import { bindPersonEvolutionForm } from './assetCenterPersonEvolution.js?v=20260822-reference-first-compact-dialogue-v135';
...
export function openAssetDrawer(item, group, handlers = {}, renderers = {}) {
  ...
  drawer.innerHTML = `<header class="drawer-head"><div><small>${escapeHtml(groupLabel)}</small><h2>${escapeHtml(item.name)}</h2></div><button class="icon-btn" type="button" data-close-drawer>×</button></header><div class="drawer-content">
    ...
    ${group === 'people' ? `<footer class="drawer-actions person-drawer-actions"><span>${item.checkpoint_recovery_summary?.retry_blocked ? '缺失单元的供应商提交或计费状态未知；成功图片已保留，核对前禁止重试。' : (readOnly ? '可查看已保存设定；生成档案是独立操作，确认后才会调用模型。' : '先保存文字设定；生成档案是另一步，生成前仍会确认并提示模型调用。')}</span><div>${readOnly ? '' : '<button class="btn" type="submit" form="personEditForm">保存人物文字设定</button>'}<button class="btn primary" type="button" data-drawer-generate ${item.checkpoint_recovery_summary?.retry_blocked ? 'disabled title="计费状态未知，禁止自动重试"' : ''}>${item.checkpoint_recovery_summary?.retry_blocked ? '等待计费核对' : (dossier ? '按最新设定重生成人物档案' : '生成完整人物档案')}</button></div></footer>` : (generatable && !dossier ? `<footer class="drawer-actions"><span>${views.length ? '可保留旧四视图，并生成新版完整档案。' : '生成前会再次展示确认，不会自动调用模型。'}</span><button class="btn primary" type="button" data-drawer-generate>生成动物资产</button></footer>` : '')}
    ...
    ${group === 'products' ? `<footer class="drawer-actions product-reference-actions"><span>${readOnly ? '当前只读；仍可基于已确认方案生成主体图片。' : '展示主体可以上传实物/材料参考，也可以先由 AI 生成一张参考图；后续场景和分镜会把它作为主体锁定素材。'}</span><div>${readOnly ? '' : `<button class="btn" type="button" data-drawer-upload-product>${item.image_url ? '更换主体图片' : '上传主体图片'}</button>`}<button class="btn primary" type="button" data-drawer-generate-product>${item.presentation?.standalone_generation_supported ? 'AI 生成商品多视图' : 'AI 生成主体参考图'}</button></div></footer>` : ''}
