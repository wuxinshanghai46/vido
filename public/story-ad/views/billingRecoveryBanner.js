export function renderCheckpointRecoveryBanner(summary = {}, escapeHtml = value => String(value || '')) {
  ...
  const copy = state === 'not_billed'
    ? { badge: '核账已完成', title: '剩余图片可以继续', detail: `剩余 ${summary.missing.length} 项未计费，只生成缺失项。`, action: `生成剩余 ${summary.missing.length} 项`, attr: 'data-history-safe data-generate-recovery' }
    : (state === 'unverifiable'
      ? { badge: '核账无法确认', title: '需要确认计费风险', detail: `先检查已有图片，再确认最多 ${summary.missing.length} 次重复计费风险。`, action: `生成剩余 ${summary.missing.length} 项`, attr: 'data-history-safe data-generate-recovery' }
      : { badge: '平台核账中', title: '人物图片已暂停', detail: `剩余 ${summary.missing.length} 项待核对；完成前不能生成。`, action: `生成剩余 ${summary.missing.length} 项`, attr: 'data-generate-recovery disabled' });
  if (planBlocked && state !== 'pending') copy.detail = `先核对已保留的 ${summary.completed} 项；可安全复用才进入费用确认。`;
