...
import { emptyState, escapeHtml, mediaPreview, setButtonBusy, toast } from '../components/ui.js?v=20260904-production-v438';
...
function flowCard(unit = {}, contract = {}) {
  return `<article class="card flow-contract-card" data-flow-beat="${escapeHtml(unit.beat_id)}">
    ...
    <div class="flow-binding-grid"><fieldset><legend>固定人物</legend>${personChecks(unit, contract.people || [])}</fieldset><label><span>固定场景</span><select data-flow-scene>${sceneOptions(unit, contract.scenes || [])}</select><small>后续镜头均使用这里确认的场景。</small></label></div>
  </article>`;
}

function historicalMarkup(rows = []) {
  if (!rows.length) return '';
  return `<details class="card legacy-flow-artifacts" data-legacy-flow-artifacts><summary>历史流向图片（${rows.length}，只读）</summary><p>旧版产物仅供追溯，不参与分镜或视频生成。</p><div class="legacy-flow-grid">${rows.map((row, index) => `<div>${mediaPreview(row, { label: `历史流向 ${index + 1}`, width: 320, symbol: '历史' })}</div>`).join('')}</div></details>`;
}
...
export async function mount(host, context) {
  ...
  host.innerHTML = `<section class="workspace-page flow-contract-page">
    <div class="page-heading flow-contract-heading"><div><h1>剧情流向确认</h1><p>确认每段剧情使用的人物和场景；这里不生成图片。</p></div><div class="flow-zero-cost"><b>零模型调用</b><span>不产生图片费用</span></div></div>
    <div class="guide flow-contract-guide" role="status"><b>${payload.gate?.ready ? '当前绑定已确认' : '确认后进入人物场景分镜'}</b><span>系统按已确认顺序和人物、场景生成镜头；缺失绑定会提前阻断。</span></div>
