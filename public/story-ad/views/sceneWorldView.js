...
import { escapeHtml, mediaPreview, toast } from '../components/ui.js?v=20260829-production-v274b';
import { promptDialog } from '../components/dialog.js?v=20260829-production-v274b';
import { list, worldById } from './sceneWorldData.js?v=20260829-production-v274b';
...
function initSceneWorldViewer({ overlay, bundle, world, authority }) {
  ...
  const showLayout = async (node, mode = 'structure') => {
    ...
    activateModeButton(mode);
    currentNode = node;
    const { mountSceneWorldLayoutViewer } = await import('./sceneWorldLayoutViewer.js?v=20260829-production-v274b');
  ...
  const showPanorama = async node => {
    ...
    try {
      const module = await import('./panoramaViewer.js?v=20260829-production-v274b');
      if (requestToken !== activation) return;
      host.replaceChildren();
...
async function openSceneWorldStudio(bundle, world) {
  const authority = await import('./sceneWorldAuthorityPlan.js?v=20260829-production-v274b');
