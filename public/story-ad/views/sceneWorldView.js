...
import { escapeHtml, mediaPreview, toast } from '../components/ui.js?v=20260823-character-library-v189';
import { promptDialog } from '../components/dialog.js?v=20260823-character-library-v189';
import { list, worldById } from './sceneWorldData.js?v=20260823-character-library-v189';
...
function initSceneWorldViewer({ overlay, bundle, world }) {
  ...
  const showPanorama = async node => {
    ...
    try {
      const module = await import('./panoramaViewer.js?v=20260823-character-library-v189');
...
async function openSceneWorldStudio(bundle, world) {
  ...
  overlay.querySelector('[data-open-director-studio]')?.addEventListener('click', async () => {
    try {
      const module = await import('./directorStudioView.js?v=20260823-character-library-v189');
...
export function bindSceneWorldWorkspace(host, bundle = {}, store = null) {
  ...
  root.querySelectorAll('[data-generate-panorama], [data-generate-all-panoramas]').forEach(button => button.addEventListener('click', async () => {
    try {
      panoramaActionModule ||= import('./panoramaGeneration.js?v=20260823-character-library-v189');
