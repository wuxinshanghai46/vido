...
import { escapeHtml, mediaPreview, toast } from '../components/ui.js?v=20260824-production-v201ae';
import { promptDialog } from '../components/dialog.js?v=20260824-production-v201ae';
import { list, worldById } from './sceneWorldData.js?v=20260824-production-v201ae';
...
function initSceneWorldViewer({ overlay, bundle, world }) {
  ...
  const showPanorama = async node => {
    ...
    try {
      const module = await import('./panoramaViewer.js?v=20260824-production-v201ae');
...
async function openSceneWorldStudio(bundle, world) {
  ...
  overlay.querySelector('[data-open-director-studio]')?.addEventListener('click', async () => {
    try {
      const module = await import('./directorStudioView.js?v=20260824-production-v201ae');
