...
import { escapeHtml, mediaPreview, toast } from '../components/ui.js?v=20260821-dialogue-interaction-v109';
import { promptDialog } from '../components/dialog.js?v=20260821-dialogue-interaction-v109';
import { list, worldById } from './sceneWorldData.js?v=20260821-dialogue-interaction-v109';
...
function initSceneWorldViewer({ overlay, bundle, world }) {
  ...
  const showPanorama = async node => {
    ...
    try {
      const module = await import('./panoramaViewer.js?v=20260821-dialogue-interaction-v109');
...
async function openSceneWorldStudio(bundle, world) {
  ...
  overlay.querySelector('[data-open-director-studio]')?.addEventListener('click', async () => {
    try {
      const module = await import('./directorStudioView.js?v=20260821-dialogue-interaction-v109');
...
export function bindSceneWorldWorkspace(host, bundle = {}, store = null) {
  ...
  root.querySelectorAll('[data-generate-panorama], [data-generate-all-panoramas]').forEach(button => button.addEventListener('click', async () => {
    try {
      panoramaActionModule ||= import('./panoramaGeneration.js?v=20260821-dialogue-interaction-v109');
