...
import { escapeHtml, mediaPreview, toast } from '../components/ui.js?v=20260830-production-v292';
import { list, worldById } from './sceneWorldData.js?v=20260830-production-v292';
import { runPanoramaGeneration } from './panoramaGeneration.js?v=20260830-production-v292';
import { capabilityChips } from './sceneWorldCapabilities.js?v=20260830-production-v292';
...
function initSceneWorldViewer({ overlay, bundle, world, authority }) {
  ...
  const showPanorama = async node => {
    ...
    try {
      const module = await import('./panoramaViewer.js?v=20260830-production-v292');
      if (requestToken !== activation) return;
      host.replaceChildren();
...
export function bindSceneWorldWorkspace(host, bundle = {}, store = null) {
  ...
  root.querySelectorAll('[data-plan-scene-experience]').forEach(button => button.addEventListener('click', async () => {
    ...
    if (!world) return;
    const { openSceneExperiencePlanner } = await import('./sceneWorldExperiencePlanner.js?v=20260830-production-v292');
