...
import { escapeHtml, mediaPreview, toast } from '../components/ui.js?v=20260825-production-v227b';
import { promptDialog } from '../components/dialog.js?v=20260825-production-v227b';
import { list, worldById } from './sceneWorldData.js?v=20260825-production-v227b';
...
function initSceneWorldViewer({ overlay, bundle, world }) {
  ...
  const showPanorama = async node => {
    ...
    try {
      const module = await import('./panoramaViewer.js?v=20260825-production-v227b');
...
async function openSceneWorldStudio(bundle, world) {
  ...
  overlay.querySelector('[data-open-director-studio]')?.addEventListener('click', async () => {
    try {
      const module = await import('./directorStudioView.js?v=20260825-production-v227b');
