...
import { escapeHtml, mediaPreview, toast } from '../components/ui.js?v=20260901-production-v379';
import { list, worldById } from './sceneWorldData.js?v=20260901-production-v379';
import { runPanoramaGeneration } from './panoramaGeneration.js?v=20260901-production-v379';
import { capabilityChips } from './sceneWorldCapabilities.js?v=20260901-production-v379';
...
function initSceneWorldViewer({ overlay, bundle, world, authority }) {
  ...
  const showLayout = async (node, mode = 'structure') => {
    ...
    activateModeButton(mode);
    currentNode = node;
    const { mountSceneWorldLayoutViewer } = await import('./sceneWorldLayoutViewer.js?v=20260901-production-v379');
  ...
  const showPanorama = async node => {
    ...
    try {
      const module = await import('./panoramaViewer.js?v=20260901-production-v379');
      if (requestToken !== activation) return;
      host.replaceChildren();
...
async function openSceneWorldStudio(bundle, world, store = null) {
  const authority = await import('./sceneWorldAuthorityPlan.js?v=20260901-production-v379');
  ...
  overlay.querySelector('[data-open-full-director]')?.addEventListener('click', async () => {
    close();
    const { openDirectorStudio } = await import('./directorStudioView.js?v=20260901-production-v379');
...
export function bindSceneWorldWorkspace(host, bundle = {}, store = null) {
  ...
  root.querySelectorAll('[data-plan-scene-experience]').forEach(button => button.addEventListener('click', async () => {
    ...
    if (!world) return;
    const { openSceneExperiencePlanner } = await import('./sceneWorldExperiencePlanner.js?v=20260901-production-v379');
