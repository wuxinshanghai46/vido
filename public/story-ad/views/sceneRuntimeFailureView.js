...
export function sceneRuntimeFailureMarkup(item = {}) {
  const failures = Object.keys(VIEW_LABELS).map(key => {
    ...
    return {
      ...
      submissionState: text(status.submission_state), message: text(status.message),
    };
  }).filter(Boolean);
  if (!failures.length) return '';
  return `<div class="scene-cover-runtime-failure" role="alert">${failures.map(failure => `<div><b>${escapeHtml(failure.label)}：${failure.state === 'billing-review' ? '计费待核对' : (failure.state === 'pending' ? '尚未提交' : '生成失败')}</b><span>${escapeHtml([failure.route, failure.httpStatus ? `HTTP ${failure.httpStatus}` : '', failure.errorCode].filter(Boolean).join(' · ') || failure.message || '供应商未返回结构化错误')}</span>${failure.platformRequestId ? `<small>平台请求：${escapeHtml(failure.platformRequestId)}</small>` : ''}${failure.providerRequestId || failure.providerTaskId ? `<small>厂商请求：${escapeHtml(failure.providerRequestId || failure.providerTaskId)}</small>` : ''}<small>提交：${escapeHtml(failure.submissionState || '未知')} · 计费：${escapeHtml(failure.billingState || '未知')}</small></div>`).join('')}</div>`;
}
