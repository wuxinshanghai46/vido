import { escapeHtml, mediaPreview, setButtonBusy, toast } from '../components/ui.js?v=20260904-production-v433';
import { sceneRuntimeFailureMarkup } from './sceneRuntimeFailureView.js?v=20260904-production-v433';
import { publicSceneQaReason, sceneQaFailureDetails, sceneQaPublicState, sceneQaRows } from './sceneQaPublicState.js?v=20260904-production-v433';

export function assetCardMedia(item = {}, group = '') {
  if (group === 'scenes') return renderSceneCoverCard(item);
  if (group === 'people') {
    const portrait = (item.native_masters?.body?.image_url ? item.native_masters.body : null)
      ...
      || (item.native_masters?.face?.image_url ? item.native_masters.face : null)
    ...
    return mediaPreview(portrait, {
      label: `${item.name || '人物'}单人物半身主视图`, width: 720,
...
export function bindSceneDossierCard(scope, item = {}) {
  ...
  button.addEventListener('click', async () => {
    try {
      ...
      const exporter = await import('./sceneDossierExport.js?v=20260904-production-v433');
