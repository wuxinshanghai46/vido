import { escapeHtml } from '../components/ui.js?v=20260810-world-person-action-contracts-v133';
...
export function briefSettingsSummary(bundle = {}) {
  ...
  return `<span class="brief-settings-values" data-brief-settings-values>
    ...
    <span class="brief-settings-meta"><em>${brief.content_mode === 'narrative_story' ? '剧情' : (brief.content_mode === 'commercial_subject' ? '广告' : '未选择类型')}</em><em>${escapeHtml(world.era_family && world.era_family !== 'auto' ? `${world.era_family}${world.time_period ? ` · ${world.time_period}` : ''}` : '世界观待识别')}</em><em>${escapeHtml(compactText(brief.product_subject || '未指定商品 / 主题', 48))}</em><em>${durationText(brief.target_duration)}</em><em>${escapeHtml(brief.output_ratio || '9:16')}</em><em>${escapeHtml(brief.video_resolution || '1080p')}</em></span>
  </span>`;
}
