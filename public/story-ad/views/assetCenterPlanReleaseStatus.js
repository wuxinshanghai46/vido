...
export function personPlanBlockedView(e={}, active=false, failure={}) {
  ...
  const progress=personPlanProgressMarkup(active,'人物方案');
  return `<section class="card asset-visual-next-step is-blocked" role="status"><div><h2>${title}</h2>${progress}${technical}</div><div class="asset-visual-next-actions"><button class="btn${active ? '' : ' primary'}" data-update-person-plan data-release-migration-only="${migration}" ${active ? 'disabled' : ''}>${button}</button></div></section>`;
}
