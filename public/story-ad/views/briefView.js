...
import { elapsedTimeTag, escapeHtml, setButtonBusy, toast } from '../components/ui.js?v=20260811-ui-v180';
import { confirmDialog, promptDialog } from '../components/dialog.js?v=20260811-ui-v180';
import { briefSettingsSummary } from './briefSettingsSummary.js?v=20260811-ui-v180';
import { worldSettingFields, worldSettingPayload } from './briefWorldSettings.js?v=20260811-ui-v180';
import { referenceProgress as renderReferenceProgress } from './referenceProgressCard.js?v=20260811-ui-v180';
...
<label class="field brief-output-field"><span>视频分辨率</span><select class="select" name="video_resolution">${['1080p', '720p', '4K'].map(value => `<option ${brief.video_resolution === value ? 'selected' : ''}>${value}</option>`).join('')}</select><small>最终导出清晰度</small></label>
  ...
  async function syncReferenceUnderstanding(reference = {}) {
    ...
    const module = await import('./referenceUnderstandingView.js?v=20260811-ui-v180');
