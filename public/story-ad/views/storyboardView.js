import { sketchBatchMarkup } from './storyboardImageReview.js?v=20260904-production-v469';
import { request } from '../api.js?v=20260904-production-v469';
import { elapsedTimeTag, emptyState, escapeHtml, mediaPreview, setButtonBusy, toast } from '../components/ui.js?v=20260904-production-v469';
...
import { generationModelPickerPlaceholder } from './generationModelPlaceholder.js?v=20260904-production-v469';
import { openStoryboardPromptEditor, referenceItemsFor, sketchReferenceMarkup } from './storyboardPromptEditorDialog.js?v=20260904-production-v469';
...
export async function mount(host, context) {
  if (context.route?.params?.get('stage') === 'shot') {
    const shotDesigner = await import('./shotDesignerView.js?v=20260904-production-v469');
  ...
  const primaryAction = !isReferenceDraft && !(completedHistorical && !sketchGate.ready)
    ...
    : '';
  host.innerHTML = `<link rel="stylesheet" href="/story-ad/storyboard-images.css?v=20260904-production-v469">
...
<div data-storyboard-live-results>${displayShots.length ? `<div class="storyboard-sketch-grid">${visibleShots.map((shot, index) => shots.length
  ...
  const startSketchBatch = async (button, options = {}) => {
    ...
    try {
      ...
      setButtonBusy(button, true, '正在提交…', { elapsed: true });
      renderSketchBatch({ status: 'queued', requested: targetCount, completed: 0, target_indexes: targetIndexes, started_at: new Date().toISOString() });
      const data = await request(`/api/story-ad/projects/${encodeURIComponent(bundle.project.id)}/storyboard-images/generate-batch`, {
        method: 'POST',
