...
import { createReferenceLinkDialogueHandler, referenceDialogueStatus, referenceNextStepDescription, routeReferenceInput, syncReferenceDialogueStatus } from './briefReferenceDialogueState.js?v=20260822-knowledge-director-learning-v137';
import { dialogueBudgetReached, referenceDialoguePhase, sanitizeDialogueTopics } from './briefDialoguePolicy.js?v=20260822-knowledge-director-learning-v137';
import { followConversationAfter } from './briefConversationScroll.js?v=20260822-knowledge-director-learning-v137';
...
export function bindBriefDialogue(host, { form, referenceState = {}, referenceAttached = false, requireUserInitiation = false, onAssist, onDialogueState, onConfirm, onReference, onReferenceLink, onProfessional } = {}) {
  ...
  const appendReferenceQuestion = async () => {
    ...
    const { mountReferenceQuestion } = await import('./briefReferenceQuestion.js?v=20260822-knowledge-director-learning-v137');
  ...
  const appendSpecificationQuestion = async () => {
    ...
    const { mountSpecificationQuestion } = await import('./briefSpecificationQuestion.js?v=20260822-knowledge-director-learning-v137');
  ...
  const submit = async () => {
    ...
    const explicitSettings = await import('./briefExplicitSettings.js?v=20260822-knowledge-director-learning-v137');
    input.value = '';
  ...
  if (!requireUserInitiation && initialReferencePhase === 'none' && String(control('brief')?.value || '').trim() && !ideaReady) {
    import('./briefGuidedResume.js?v=20260822-knowledge-director-learning-v137').then(({ guidedResumePrompt }) => {
      if (disposed) return;
