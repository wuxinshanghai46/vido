...
export function bindBriefDialogue(host, { form, referenceAttached = false, requireUserInitiation = false, onAssist, onConfirm, onReference, onReferenceLink, onProfessional } = {}) {
  ...
  const appendReferenceQuestion = async () => {
    ...
    const { mountReferenceQuestion } = await import('./briefReferenceQuestion.js?v=20260821-dialogue-v123');
  ...
  const appendSpecificationQuestion = async () => {
    ...
    const { mountSpecificationQuestion } = await import('./briefSpecificationQuestion.js?v=20260821-dialogue-v123');
  ...
  const submit = async () => {
    ...
    const explicitSettings = await import('./briefExplicitSettings.js?v=20260821-dialogue-v123');
    input.value = '';
  ...
  if (!requireUserInitiation && String(control('brief')?.value || '').trim() && !ideaReady) {
    import('./briefGuidedResume.js?v=20260821-dialogue-v123').then(({ guidedResumePrompt }) => {
      if (disposed) return;
