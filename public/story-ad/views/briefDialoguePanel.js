import { escapeHtml } from '../components/ui.js?v=20260827-production-v235c';
import { createReferenceLinkDialogueHandler, referenceDialogueStatus, referenceNextStepDescription, routeReferenceInput, syncReferenceDialogueStatus } from './briefReferenceDialogueState.js?v=20260827-production-v235c';
import { allowedDialogueTopics,referenceDialoguePhase,sanitizeDialogueTopics } from './briefDialoguePolicy.js?v=20260827-production-v235c';
import { followConversationAfter } from './briefConversationScroll.js?v=20260827-production-v235c';
import { appendDialogueSuggestions,briefIdeaPreview,contextualDialogueFallback,dialogueHistoryMarkup,ideaMarkup,modeLabel,normalizedDialogueHistory,recordDialogueHistory } from './briefDialogueProjection.js?v=20260827-production-v235c';
import { dialogueIntakeState, dialogueProgressState } from './briefDialogueReadiness.js?v=20260827-production-v235c';
...
export function bindBriefDialogue(host, { form, referenceState={}, referenceAttached=false, requireUserInitiation=false, onAssist, onDialogueState, onConfirm, onReference, onReferenceLink, onProfessional }={}) {
  ...
  const appendReferenceQuestion = async () => {
    ...
    const { mountReferenceQuestion, referenceQuestionText } = await import('./briefReferenceQuestion.js?v=20260827-production-v235c');
  ...
  const appendSpecificationQuestion = async () => {
    ...
    const { mountSpecificationQuestion, specificationQuestionText } = await import('./briefSpecificationQuestion.js?v=20260827-production-v235c');
  ...
  const appendCastQuestion = async () => {
    ...
    const { mountCastQuestion, castQuestionText } = await import('./briefCastQuestion.js?v=20260827-production-v235c');
  ...
  const submit = async () => {
    ...
    const explicitSettings = await import('./briefExplicitSettings.js?v=20260827-production-v235c');
    input.value = '';
  ...
  if (!requireUserInitiation && initialReferencePhase === 'none' && String(control('brief')?.value || '').trim() && !ideaReady) {
    import('./briefGuidedResume.js?v=20260827-production-v235c').then(({ guidedResumePrompt }) => {
      if (disposed) return;
