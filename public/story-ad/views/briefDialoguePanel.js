import { escapeHtml } from '../components/ui.js?v=20260904-production-v462';
import { createReferenceLinkDialogueHandler, referenceDialogueStatus, referenceNextStepDescription, routeReferenceInput, syncReferenceDialogueStatus } from './briefReferenceDialogueState.js?v=20260904-production-v462';
import { allowedDialogueTopics,referenceDialoguePhase,sanitizeDialogueTopics } from './briefDialoguePolicy.js?v=20260904-production-v462';
import { followConversationAfter } from './briefConversationScroll.js?v=20260904-production-v462';
import { appendDialogueSuggestions,briefIdeaPreview,contextualDialogueFallback,dialogueHistoryMarkup,ideaMarkup,modeLabel,normalizedDialogueHistory,recordDialogueHistory } from './briefDialogueProjection.js?v=20260904-production-v462';
import { dialogueIntakeState, dialogueProgressState } from './briefDialogueReadiness.js?v=20260904-production-v462';
  ...
  return `<section class="brief-dialogue" data-brief-dialogue>
    <div class="brief-conversation-panel">
      ...
      <div class="brief-conversation-scroll" data-brief-conversation aria-live="polite">
        ...
        <div class="brief-reference-progress-slot" data-reference-progress-host>${options.referenceProgressMarkup || ''}</div>
      </div>
...
export function bindBriefDialogue(host, { form, referenceState={}, referenceAttached=false, requireUserInitiation=false, onAssist, onDialogueState, onConfirm, onReference, onReferenceLink, onProfessional }={}) {
  ...
  const appendReferenceQuestion = async () => {
    ...
    const { mountReferenceQuestion, referenceQuestionText } = await import('./briefReferenceQuestion.js?v=20260904-production-v462');
  ...
  const appendSpecificationQuestion = async () => {
    ...
    const { mountSpecificationQuestion, specificationQuestionText } = await import('./briefSpecificationQuestion.js?v=20260904-production-v462');
    recordHistory('assistant', specificationQuestionText({ mode: String(control('content_mode')?.value || ''), duration: Number(control('target_duration')?.value || 30) || 30, ratio: String(control('output_ratio')?.value || '9:16'), resolution: String(control('video_resolution')?.value || '480p') }), { topic: 'specifications' });
    mountSpecificationQuestion(conversation, {
      ...
      resolution: String(control('video_resolution')?.value || '480p'),
      onConfirm: async () => {
  ...
  const appendCastQuestion = async () => {
    ...
    const { mountCastQuestion, castQuestionText } = await import('./briefCastQuestion.js?v=20260904-production-v462');
  ...
  const sync = () => {
    ...
    const resolution = String(control('video_resolution')?.value || '480p');
  ...
  const submit = async () => {
    ...
    const explicitSettings = await import('./briefExplicitSettings.js?v=20260904-production-v462');
    input.value = '';
    ...
    try {
      ...
      const result = await onAssist?.({
        ...
        video_resolution: String(control('video_resolution')?.value || '480p'),
        specifications_confirmed: specificationsConfirmed,
  ...
  if (!requireUserInitiation && initialReferencePhase === 'none' && String(control('brief')?.value || '').trim() && !ideaReady) {
    import('./briefGuidedResume.js?v=20260904-production-v462').then(({ guidedResumePrompt }) => {
      if (disposed) return;
