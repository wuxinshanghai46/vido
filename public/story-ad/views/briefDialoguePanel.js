import { escapeHtml } from '../components/ui.js?v=20260904-production-v439';
import { createReferenceLinkDialogueHandler, referenceDialogueStatus, referenceNextStepDescription, routeReferenceInput, syncReferenceDialogueStatus } from './briefReferenceDialogueState.js?v=20260904-production-v439';
import { allowedDialogueTopics,referenceDialoguePhase,sanitizeDialogueTopics } from './briefDialoguePolicy.js?v=20260904-production-v439';
import { followConversationAfter } from './briefConversationScroll.js?v=20260904-production-v439';
import { appendDialogueSuggestions,briefIdeaPreview,contextualDialogueFallback,dialogueHistoryMarkup,ideaMarkup,modeLabel,normalizedDialogueHistory,recordDialogueHistory } from './briefDialogueProjection.js?v=20260904-production-v439';
import { dialogueIntakeState, dialogueProgressState } from './briefDialogueReadiness.js?v=20260904-production-v439';
...
export function bindBriefDialogue(host, { form, referenceState={}, referenceAttached=false, requireUserInitiation=false, onAssist, onDialogueState, onConfirm, onReference, onReferenceLink, onProfessional }={}) {
  ...
  const appendReferenceQuestion = async () => {
    ...
    const { mountReferenceQuestion, referenceQuestionText } = await import('./briefReferenceQuestion.js?v=20260904-production-v439');
  ...
  const appendSpecificationQuestion = async () => {
    ...
    const { mountSpecificationQuestion, specificationQuestionText } = await import('./briefSpecificationQuestion.js?v=20260904-production-v439');
  ...
  const appendCastQuestion = async () => {
    ...
    const { mountCastQuestion, castQuestionText } = await import('./briefCastQuestion.js?v=20260904-production-v439');
  ...
  const submit = async () => {
    ...
    const explicitSettings = await import('./briefExplicitSettings.js?v=20260904-production-v439');
    input.value = '';
  ...
  if (!requireUserInitiation && initialReferencePhase === 'none' && String(control('brief')?.value || '').trim() && !ideaReady) {
    import('./briefGuidedResume.js?v=20260904-production-v439').then(({ guidedResumePrompt }) => {
      if (disposed) return;
