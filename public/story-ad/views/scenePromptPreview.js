import { escapeHtml, toast } from '../components/ui.js?v=20260827-production-v233i';
import { renderSceneCoverCard, sceneNeedsGeneration } from './sceneDossierCard.js?v=20260827-production-v233i';
...
export function renderSceneProductionCard(scene = {}, index = 0, options = {}) {
  ...
  return `<article class="scene-production-card" data-scene-card data-scene-id="${sceneId}" data-prompt-version-id="${escapeHtml(promptState.prompt_version_id || '')}" data-default-scene-tab="${preferredTab}">
    ...
    <footer><span>${provisional ? '正式规划完成后可生成画面' : (options.generationActive ? '该场景正在生成，其他场景仍可继续提交' : (needsGeneration ? '编辑会自动保存，可直接生成场景画面' : '画面已就绪，可按需重新生成'))}</span>${!provisional ? `<div class="scene-image-settings"><select data-scene-quality aria-label="场景画质"><option value="low">低画质</option><option value="standard" selected>标准画质</option><option value="high">高画质</option></select><select data-scene-resolution aria-label="场景清晰度"><option value="1K">1K</option><option value="2K" selected>2K</option><option value="4K" disabled>4K（当前模型不支持）</option></select><button class="btn primary compact" data-generate-scene="${sceneId}" ${options.generationActive ? 'disabled' : ''}>${options.generationActive ? '正在生成…' : (needsGeneration ? '生成该场景' : '重新生成')}</button></div>` : ''}</footer>
  </article>`;
}
