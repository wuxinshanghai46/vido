import { request, uploadAsset, uploadReferenceVideo } from '../api.js?v=20260828-production-v245';
import { beginReferenceReplacement, referenceSyncInterrupted, replacementCurrent, removeProjectReference, restoreReferenceReplacement } from './referenceReplacementState.js?v=20260828-production-v245';
import { cancelReferenceAnalysisRequest, retryReferenceAnalysisRequest, retryReferenceImportRequest } from './referenceRetryStore.js?v=20260828-production-v245';
import { loadProjectList } from './projectListStore.js?v=20260828-production-v245';
import { loadProjectBundle, refreshProjectBundle } from './projectBundleStore.js?v=20260828-production-v245';
import { beginStageSubmissionState } from './stageSubmissionState.js?v=20260828-production-v245';
export function createProjectStore() {
  ...
  const mediaStore = () => import('./mediaCatalogStore.js?v=20260828-production-v245'), loadMediaPage = async options => (await mediaStore()).loadMediaPage({ request, state }, options);
  ...
  return {
    ...
    runStage,
    saveScenePrompt: async (scene, prompt) => (await import('./scenePromptConfirmationStore.js?v=20260828-production-v245')).saveScenePrompt({ state, request }, scene, prompt),
    saveBlueprint,
    saveStoryboard,
    saveSketches,
    upload,
