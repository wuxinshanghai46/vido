import { request, uploadAsset, uploadReferenceVideo } from '../api.js?v=20260828-production-v240c';
import { beginReferenceReplacement, referenceSyncInterrupted, replacementCurrent, removeProjectReference, restoreReferenceReplacement } from './referenceReplacementState.js?v=20260828-production-v240c';
import { cancelReferenceAnalysisRequest, retryReferenceAnalysisRequest, retryReferenceImportRequest } from './referenceRetryStore.js?v=20260828-production-v240c';
import { loadProjectList } from './projectListStore.js?v=20260828-production-v240c';
import { loadProjectBundle, refreshProjectBundle } from './projectBundleStore.js?v=20260828-production-v240c';
import { beginStageSubmissionState } from './stageSubmissionState.js?v=20260828-production-v240c';
export function createProjectStore() {
  ...
  const mediaStore = () => import('./mediaCatalogStore.js?v=20260828-production-v240c'), loadMediaPage = async options => (await mediaStore()).loadMediaPage({ request, state }, options);
  ...
  return {
    ...
    runStage,
    saveScenePrompt: async (scene, prompt) => (await import('./scenePromptConfirmationStore.js?v=20260828-production-v240c')).saveScenePrompt({ state, request }, scene, prompt),
    saveBlueprint,
    saveStoryboard,
    saveSketches,
    upload,
