import { request, uploadAsset, uploadReferenceVideo } from '../api.js?v=20260830-production-v291b';
import { beginReferenceReplacement, referenceSyncInterrupted, replacementCurrent, removeProjectReference, restoreReferenceReplacement } from './referenceReplacementState.js?v=20260830-production-v291b';
import { cancelReferenceAnalysisRequest, retryReferenceAnalysisRequest, retryReferenceImportRequest } from './referenceRetryStore.js?v=20260830-production-v291b';
import { loadProjectList } from './projectListStore.js?v=20260830-production-v291b';
import { loadProjectBundle, prefetchProjectBundle, refreshProjectBundle } from './projectBundleStore.js?v=20260830-production-v291b';
import { beginStageSubmissionState } from './stageSubmissionState.js?v=20260830-production-v291b';
import { createStoryboardLiveRefresh } from './storyboardLiveRefresh.js?v=20260830-production-v291b';
export function createProjectStore() {
  ...
  const mediaStore = () => import('./mediaCatalogStore.js?v=20260830-production-v291b'), loadMediaPage = async options => (await mediaStore()).loadMediaPage({ request, state }, options);
  ...
  return {
    ...
    runStage,
    saveScenePrompt: async (scene, prompt) => (await import('./scenePromptConfirmationStore.js?v=20260830-production-v291b')).saveScenePrompt({ state, request }, scene, prompt),
    saveBlueprint,
    saveStoryboard,
    ...
    startVideo,
    cancelGeneration,
    acceptCurrentScenes: async () => (await import('./sceneAcceptanceStore.js?v=20260830-production-v291b')).acceptCurrentScenes({ state, request, refreshSections }),
    clearProject,
    syncProgressPolling,
    stopProgressPolling,
    stopReferencePolling,
