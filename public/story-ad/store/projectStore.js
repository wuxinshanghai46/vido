import { request, uploadAsset, uploadReferenceVideo } from '../api.js?v=20260827-production-v237c';
import { beginReferenceReplacement, referenceSyncInterrupted, replacementCurrent, removeProjectReference, restoreReferenceReplacement } from './referenceReplacementState.js?v=20260827-production-v237c';
import { cancelReferenceAnalysisRequest, retryReferenceAnalysisRequest, retryReferenceImportRequest } from './referenceRetryStore.js?v=20260827-production-v237c';
import { loadProjectList } from './projectListStore.js?v=20260827-production-v237c';
import { loadProjectBundle, refreshProjectBundle } from './projectBundleStore.js?v=20260827-production-v237c';
export function createProjectStore() {
  ...
  const mediaStore = () => import('./mediaCatalogStore.js?v=20260827-production-v237c'), loadMediaPage = async options => (await mediaStore()).loadMediaPage({ request, state }, options);
  ...
  return {
    ...
    updateRequest,
    runStage,
    saveScenePrompt: async (scene, prompt) => (await import('./scenePromptConfirmationStore.js?v=20260827-production-v237c')).saveScenePrompt({ state, request }, scene, prompt),
    saveBlueprint,
    saveStoryboard,
    saveSketches,
    upload,
