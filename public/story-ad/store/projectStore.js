import { request, uploadAsset, uploadReferenceVideo } from '../api.js?v=20260827-production-v238';
import { beginReferenceReplacement, referenceSyncInterrupted, replacementCurrent, removeProjectReference, restoreReferenceReplacement } from './referenceReplacementState.js?v=20260827-production-v238';
import { cancelReferenceAnalysisRequest, retryReferenceAnalysisRequest, retryReferenceImportRequest } from './referenceRetryStore.js?v=20260827-production-v238';
import { loadProjectList } from './projectListStore.js?v=20260827-production-v238';
import { loadProjectBundle, refreshProjectBundle } from './projectBundleStore.js?v=20260827-production-v238';
export function createProjectStore() {
  ...
  const mediaStore = () => import('./mediaCatalogStore.js?v=20260827-production-v238'), loadMediaPage = async options => (await mediaStore()).loadMediaPage({ request, state }, options);
  ...
  return {
    ...
    updateRequest,
    runStage,
    saveScenePrompt: async (scene, prompt) => (await import('./scenePromptConfirmationStore.js?v=20260827-production-v238')).saveScenePrompt({ state, request }, scene, prompt),
    saveBlueprint,
    saveStoryboard,
    saveSketches,
    upload,
