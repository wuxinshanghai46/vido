import { request, uploadAsset, uploadReferenceVideo } from '../api.js?v=20260828-production-v244';
import { beginReferenceReplacement, referenceSyncInterrupted, replacementCurrent, removeProjectReference, restoreReferenceReplacement } from './referenceReplacementState.js?v=20260828-production-v244';
import { cancelReferenceAnalysisRequest, retryReferenceAnalysisRequest, retryReferenceImportRequest } from './referenceRetryStore.js?v=20260828-production-v244';
import { loadProjectList } from './projectListStore.js?v=20260828-production-v244';
import { loadProjectBundle, refreshProjectBundle } from './projectBundleStore.js?v=20260828-production-v244';
import { beginStageSubmissionState } from './stageSubmissionState.js?v=20260828-production-v244';
export function createProjectStore() {
  ...
  const mediaStore = () => import('./mediaCatalogStore.js?v=20260828-production-v244'), loadMediaPage = async options => (await mediaStore()).loadMediaPage({ request, state }, options);
    ...
    const poll = async () => {
      ...
      try {
        ...
        const project = {
          ...
          active_generation_id: progressTask.active_generation_id || '', active_target_generations: progressTask.active_target_generations || {},
          target_generation_progress: progressTask.target_generation_progress || {},
  ...
  return {
    ...
    runStage,
    saveScenePrompt: async (scene, prompt) => (await import('./scenePromptConfirmationStore.js?v=20260828-production-v244')).saveScenePrompt({ state, request }, scene, prompt),
    saveBlueprint,
    saveStoryboard,
    saveSketches,
    upload,
