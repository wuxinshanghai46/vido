import { request, uploadAsset, uploadReferenceVideo } from '../api.js?v=20260904-production-v447';
import { beginReferenceReplacement, referenceSyncInterrupted, replacementCurrent, removeProjectReference, restoreReferenceReplacement } from './referenceReplacementState.js?v=20260904-production-v447';
import { cancelReferenceAnalysisRequest, retryReferenceAnalysisRequest, retryReferenceImportRequest } from './referenceRetryStore.js?v=20260904-production-v447';
import { loadProjectList } from './projectListStore.js?v=20260904-production-v447';
import { loadProjectBundle, prefetchProjectBundle, refreshProjectBundle } from './projectBundleStore.js?v=20260904-production-v447';
import { beginAutomaticStageSubmission, beginStageSubmissionState } from './stageSubmissionState.js?v=20260904-production-v447';
import { createStoryboardLiveRefresh } from './storyboardLiveRefresh.js?v=20260904-production-v447';
import { retainActiveGenerationProgress } from './progressProjection.js?v=20260904-production-v447';
export function createProjectStore() {
  ...
  const mediaStore = () => import('./mediaCatalogStore.js?v=20260904-production-v447'), loadMediaPage = async options => (await mediaStore()).loadMediaPage({ request, state }, options);
  ...
  return {
    ...
    runStage,
    saveScenePrompt: async (scene, prompt) => (await import('./scenePromptConfirmationStore.js?v=20260904-production-v447')).saveScenePrompt({ state, request }, scene, prompt),
    saveBlueprint,
    saveStoryboard,
    ...
    startVideo,
    cancelGeneration,
    acceptCurrentScenes: async () => (await import('./sceneAcceptanceStore.js?v=20260904-production-v447')).acceptCurrentScenes({ state, request, refreshSections }),
    clearProject,
    syncProgressPolling,
    stopProgressPolling,
    stopReferencePolling,
