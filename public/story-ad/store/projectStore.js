import { request, uploadAsset, uploadReferenceVideo } from '../api.js?v=20260904-production-v463';
import { beginReferenceReplacement, referenceSyncInterrupted, replacementCurrent, removeProjectReference, restoreReferenceReplacement } from './referenceReplacementState.js?v=20260904-production-v463';
import { cancelReferenceAnalysisRequest, retryReferenceAnalysisRequest, retryReferenceImportRequest } from './referenceRetryStore.js?v=20260904-production-v463';
import { loadProjectList } from './projectListStore.js?v=20260904-production-v463';
import { loadProjectBundle, prefetchProjectBundle, refreshProjectBundle } from './projectBundleStore.js?v=20260904-production-v463';
import { beginAutomaticStageSubmission, beginStageSubmissionState } from './stageSubmissionState.js?v=20260904-production-v463';
import { createStoryboardLiveRefresh } from './storyboardLiveRefresh.js?v=20260904-production-v463';
import { retainActiveGenerationProgress } from './progressProjection.js?v=20260904-production-v463';
export function createProjectStore() {
  const state = { projects: [],
    stats: {},
    projectListScope: 'current_user',
    bundle: null,
    bundleSections: [],
    bundleRequestSeq: 0,
    loading: false,
  ...
  const mediaStore = () => import('./mediaCatalogStore.js?v=20260904-production-v463'), loadMediaPage = async options => (await mediaStore()).loadMediaPage({ request, state }, options);
  ...
  async function runStage(path, body = {}) {
    const taskId = state.bundle?.project?.id;
    if (!taskId) throw new Error('请先创建项目。');
    beginAutomaticStageSubmission({ state, set }, path, body);
    set({ saving: true, error: '' });
  ...
  async function attachMaterial(role, asset, options = {}) {
    ...
    await loadBundle(taskId, 'summary,reference,assets');
    return data;
  }
  ...
  return {
    ...
    runStage,
    saveScenePrompt: async (scene, prompt) => (await import('./scenePromptConfirmationStore.js?v=20260904-production-v463')).saveScenePrompt({ state, request }, scene, prompt),
    saveBlueprint,
    saveStoryboard,
    ...
    startVideo,
    cancelGeneration,
    acceptCurrentScenes: async () => (await import('./sceneAcceptanceStore.js?v=20260904-production-v463')).acceptCurrentScenes({ state, request, refreshSections }),
    clearProject,
    syncProgressPolling,
    stopProgressPolling,
    stopReferencePolling,
