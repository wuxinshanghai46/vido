import { request, uploadAsset, uploadReferenceVideo } from '../api.js?v=20260827-production-v238d';
import { beginReferenceReplacement, referenceSyncInterrupted, replacementCurrent, removeProjectReference, restoreReferenceReplacement } from './referenceReplacementState.js?v=20260827-production-v238d';
import { cancelReferenceAnalysisRequest, retryReferenceAnalysisRequest, retryReferenceImportRequest } from './referenceRetryStore.js?v=20260827-production-v238d';
import { loadProjectList } from './projectListStore.js?v=20260827-production-v238d';
import { loadProjectBundle, refreshProjectBundle } from './projectBundleStore.js?v=20260827-production-v238d';
export function createProjectStore() {
  ...
  const mediaStore = () => import('./mediaCatalogStore.js?v=20260827-production-v238d'), loadMediaPage = async options => (await mediaStore()).loadMediaPage({ request, state }, options);
  ...
  return {
    ...
    updateRequest,
    runStage,
    saveScenePrompt: async (scene, prompt) => (await import('./scenePromptConfirmationStore.js?v=20260827-production-v238d')).saveScenePrompt({ state, request }, scene, prompt),
    saveBlueprint,
    saveStoryboard,
    saveSketches,
    upload,
