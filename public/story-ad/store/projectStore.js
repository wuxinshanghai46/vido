import { request, uploadAsset, uploadReferenceVideo } from '../api.js?v=20260830-production-v289';
import { beginReferenceReplacement, referenceSyncInterrupted, replacementCurrent, removeProjectReference, restoreReferenceReplacement } from './referenceReplacementState.js?v=20260830-production-v289';
import { cancelReferenceAnalysisRequest, retryReferenceAnalysisRequest, retryReferenceImportRequest } from './referenceRetryStore.js?v=20260830-production-v289';
import { loadProjectList } from './projectListStore.js?v=20260830-production-v289';
import { loadProjectBundle, prefetchProjectBundle, refreshProjectBundle } from './projectBundleStore.js?v=20260830-production-v289';
import { beginStageSubmissionState } from './stageSubmissionState.js?v=20260830-production-v289';
export function createProjectStore() {
  ...
  const mediaStore = () => import('./mediaCatalogStore.js?v=20260830-production-v289'), loadMediaPage = async options => (await mediaStore()).loadMediaPage({ request, state }, options);
  ...
  return {
    ...
    runStage,
    saveScenePrompt: async (scene, prompt) => (await import('./scenePromptConfirmationStore.js?v=20260830-production-v289')).saveScenePrompt({ state, request }, scene, prompt),
    saveBlueprint,
    saveStoryboard,
    ...
    startVideo,
    cancelGeneration,
    acceptCurrentScenes: async () => (await import('./sceneAcceptanceStore.js?v=20260830-production-v289')).acceptCurrentScenes({ state, request, refreshSections }),
    clearProject,
    syncProgressPolling,
    stopProgressPolling,
    stopReferencePolling,
